...
const [
    ...
    window.__gmCoreAuthManaged = true;
    window.db = db;
    window.auth = auth;
    window.app = app;
    window.currentUser = null;
    window.currentViewId = 'view-auth';
    document.body.dataset.activeView = 'view-auth';
    window.actionRateState = {};

    window.STORY_PIECE_SETS = [
        { id: 'neo', name: 'Neo Klasik', price: 0, theme: 'neo', accent: '#d4af37', rarity: 'Başlangıç' },
        { id: 'marble', name: 'Mermer Saray', price: 250, theme: 'marble', accent: '#e5e7eb', rarity: 'Usta' },
        { id: 'emerald', name: 'Cam Zümrüt', price: 450, theme: 'glass', accent: '#34d399', rarity: 'Nadir' },
        { id: 'obsidian', name: 'Gece Obsidyeni', price: 700, theme: 'gothic', accent: '#a78bfa', rarity: 'Epik' },
        { id: 'ruby', name: 'Turnuva Kasası', price: 950, theme: 'cases', accent: '#fb7185', rarity: 'Epik' },
        { id: 'gold', name: 'Altın Hanedan', price: 1300, theme: 'metal', accent: '#facc15', rarity: 'Efsane' },
        { id: 'cosmic', name: 'Kozmik Taç', price: 1700, theme: 'space', accent: '#38bdf8', rarity: 'Mitik' }
    ];

    window.getActivePieceSet = function() {
        const owned = window.currentProfileData && Array.isArray(window.currentProfileData.storyOwnedPieceSets)
            ? window.currentProfileData.storyOwnedPieceSets
            : ['neo'];
        const active = window.currentProfileData && window.currentProfileData.activePieceSet
            ? window.currentProfileData.activePieceSet
            : 'neo';
        const set = window.STORY_PIECE_SETS.find(function(item) { return item.id === active && owned.indexOf(item.id) !== -1; });
        return set || window.STORY_PIECE_SETS[0];
    };

    window.getPieceAsset = function(color, type) {
        const pieceColor = color === 'b' ? 'b' : 'w';
        const pieceType = ['p', 'n', 'b', 'r', 'q', 'k'].indexOf(type) !== -1 ? type : 'p';
        const activeSet = window.getActivePieceSet();
        return 'https://images.chesscomfiles.com/chess-themes/pieces/' + activeSet.theme + '/150/' + pieceColor + pieceType + '.png';
    };

    window.getPieceFallbackAsset = function(color, type) {
        const pieceColor = color === 'b' ? 'b' : 'w';
        const pieceType = ['p', 'n', 'b', 'r', 'q', 'k'].indexOf(type) !== -1 ? type : 'p';
        return 'https://images.chesscomfiles.com/chess-themes/pieces/neo/150/' + pieceColor + pieceType + '.png';
    };

    window.applyPieceSkin = function(pieceEl, color, type) {
        if (!pieceEl) return;
        const primary = window.getPieceAsset(color, type);
        pieceEl.style.backgroundImage = "url('" + primary + "')";
        pieceEl.style.backgroundSize = 'cover';
    };

    // Load modules dynamically
    const cacheBuster = '20260924-items2';
    await Promise.all([
    ...
    const soundGenerators = {
        ...
        promotion: function() {
            playTone(440, 0.07, 'triangle', 0.12);
            setTimeout(function(){ playTone(660, 0.08, 'triangle', 0.14); }, 70);
            setTimeout(function(){ playTone(880, 0.11, 'triangle', 0.16); }, 150);
        },
        checkmate: function() {
            playNoise(0.16, 0.32, 420);
            playTone(740, 0.12, 'square', 0.16);
            setTimeout(function(){ playTone(554, 0.16, 'square', 0.14); }, 120);
            setTimeout(function(){ playTone(370, 0.2, 'sawtooth', 0.12); }, 270);
        },
        notify: function() {
    ...
    window.playChessMoveSound = function(move, game) {
        if (!move) return window.playGameSound('move');
        const flags = move.flags || '';
        const isMate = game && typeof game.in_checkmate === 'function' && game.in_checkmate();
        const isCheck = game && typeof game.in_check === 'function' && game.in_check();
        const isCapture = !!move.captured || flags.indexOf('c') !== -1 || flags.indexOf('e') !== -1;
        const isCastle = flags.indexOf('k') !== -1 || flags.indexOf('q') !== -1;
        const isPromotion = !!move.promotion || flags.indexOf('p') !== -1;
        if (isMate) return window.playGameSound('checkmate');
        if (isCheck) return window.playGameSound('check');
        if (isCapture) return window.playGameSound('capture');
        if (isCastle) return window.playGameSound('castle');
        if (isPromotion) return window.playGameSound('promotion');
        return window.playGameSound('move');
    };

    window.playChessMoveSoundFromFlags = function(flags, game, isCheckFlag) {
        const moveFlags = flags || '';
        const isMate = game && typeof game.in_checkmate === 'function' && game.in_checkmate();
        const isCheck = isCheckFlag === true || (game && typeof game.in_check === 'function' && game.in_check());
        const isCapture = moveFlags.indexOf('c') !== -1 || moveFlags.indexOf('e') !== -1;
        const isCastle = moveFlags.indexOf('k') !== -1 || moveFlags.indexOf('q') !== -1;
        const isPromotion = moveFlags.indexOf('p') !== -1;
        if (isMate) return window.playGameSound('checkmate');
        if (isCheck) return window.playGameSound('check');
        if (isCapture) return window.playGameSound('capture');
        if (isCastle) return window.playGameSound('castle');
    ...
    function scheduleFullscreenBoardPaint() {
        if (!window.refreshFullscreenBoard) return;
        window.refreshFullscreenBoard();
        requestAnimationFrame(function() {
            if (window.refreshFullscreenBoard) window.refreshFullscreenBoard();
        });
    }
    ...
    window.syncBoardFullscreenUI = function() {
        ...
        if (activeFullscreenBoardMode === '1v1') {
            ...
            const activeColor = current1v1Data.status === 'active' ? (window.chess1v1.turn() === 'w' ? 'white' : 'black') : null;
        ...
        if (activeFullscreenBoardMode === '2v2') {
            ...
            const activeColor = current2v2Data.status === 'active' ? (window.chess.turn() === 'w' ? 'white' : 'black') : null;
    ...
    window.openBoardFullscreen = function(mode) {
        if (mode !== '1v1' && mode !== '2v2') return;
        if (mode === '1v1' && !window.current1v1Data) {
            window.showToast('Tam ekran için aktif 1v1 oyunu gerekli.', 'error');
            return;
        }
        if (mode === '2v2' && !window.current2v2Data) {
            window.showToast('Tam ekran için aktif 2v2 oyunu gerekli.', 'error');
            return;
        }
    ...
    document.addEventListener('fullscreenchange', function() {
        ...
        if (document.fullscreenElement !== overlay && window.activeFullscreenBoardMode && overlay.classList.contains('active')) {
            window.activeFullscreenBoardMode = null;
            setBoardFullscreenVisible(false);
        }
    });
    ...
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape' && window.activeFullscreenBoardMode && !document.fullscreenElement) {
            window.closeBoardFullscreen();
        }
    });

    window.addEventListener('resize', function() {
        if (window.activeFullscreenBoardMode && window.refreshFullscreenBoard) {
            window.refreshFullscreenBoard();
        }
    });
    ...
    function markAvatarSelection(container, value) {
        if (!container) return;
        container.querySelectorAll('.avatar-option').forEach(function(option) {
            option.classList.toggle('selected', option.dataset.avatar === value);
        });
    }

    function syncSettingsFormFromCurrentUser() {
        const nameInput = document.getElementById('settingsNameInput');
        const avatarInput = document.getElementById('settingsSelectedAvatar');
        const avatarGrid = document.getElementById('settingsAvatarGrid');
        const muteToggle = document.getElementById('muteSoundToggle');
        const activeUser = window.currentUser || auth.currentUser;
        const displayName = (activeUser && activeUser.displayName) || '';
        const avatar = (activeUser && activeUser.photoURL) || 'fa-chess-pawn';

        if (nameInput) nameInput.value = displayName;
        if (avatarInput) avatarInput.value = avatar;
        markAvatarSelection(avatarGrid, avatar);
        if (muteToggle) muteToggle.checked = localStorage.getItem('gm_mute') !== 'true';
        const langSelect = document.getElementById('settingsAnalysisLang');
    ...
    window.GM_APP_BASE_URL = window.GM_APP_BASE_URL || 'http://localhost:8080';
    window.getAppBaseUrl = function() {
        const origin = window.location.origin;
        let path = window.location.pathname || '/';
        if (window.location.protocol === 'file:' || !origin || origin === 'null') {
            return window.GM_APP_BASE_URL.replace(/\/$/, '') + '/';
        }
    ...
    function capturePendingDeepLinkFromUrl() {
        try {
            const url = new URL(window.location.href);
            const share = url.searchParams.get('share');
            const watchMode = url.searchParams.get('watchMode');
            const watchCode = url.searchParams.get('watchCode');
            if (!share && !(watchMode && watchCode)) return;
            sessionStorage.setItem('gm_pending_deeplink', JSON.stringify({
                share: share || null,
                watchMode: watchMode || null,
                watchCode: watchCode || null
            }));
            const toDelete = [];
            if (share) toDelete.push('share');
            if (watchMode) toDelete.push('watchMode', 'watchCode');
            replaceUrlParams(toDelete);
        } catch (e) {}
    }

    window.processPendingDeepLink = function() {
        try {
            const raw = sessionStorage.getItem('gm_pending_deeplink');
            if (!raw) return false;
            const data = JSON.parse(raw);
            if (data.share && window.openSharedAnalysisById) {
                sessionStorage.removeItem('gm_pending_deeplink');
                window.openSharedAnalysisById(data.share);
                return true;
            }
            if (data.watchMode && data.watchCode) {
                if (!window.currentUser) {
                    window.showToast('Maçı izlemek için önce giriş yapın.', 'info');
                    return false;
                }
                sessionStorage.removeItem('gm_pending_deeplink');
                const code = String(data.watchCode).trim().toUpperCase();
                if (data.watchMode === '1v1' && window.enter1v1Game) {
                    window.enter1v1Game(code);
                    return true;
                }
                if (data.watchMode === '2v2' && window.enter2v2Game) {
    ...
    capturePendingDeepLinkFromUrl();

    window.makeId = function(length) { let result = ''; const characters = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; for (let i = 0; i < length; i++) result += characters.charAt(Math.floor(Math.random() * characters.length)); return result; };
    ...
    window.rememberLocalActiveGame = function(mode, code, extra) {
        if (!mode || !code) return;
        try {
            localStorage.setItem(LAST_ACTIVE_GAME_KEY, window.buildLocalActiveGamePayload(mode, code, extra));
        } catch (e) {}
    };

    window.markLocalActiveGameDisconnected = function(mode, code, disconnectedAtMs) {
        if (!mode || !code) return;
    ...
    window.getCurrentReconnectContext = function() {
        if (window.current1v1Id && window.current1v1Data && window.current1v1Data.status === 'active' && window.current1v1Role === 'player') {
            return { mode: '1v1', id: window.current1v1Id, data: window.current1v1Data };
        }
        if (window.current2v2Id && window.current2v2Data && window.current2v2Data.status === 'active' && window.current2v2Role === 'player') {
            return { mode: '2v2', id: window.current2v2Id, data: window.current2v2Data };
        }
        return null;
    };
    ...
    let currentCRDeck = [];
    window.openClashGenerator = () => { window.playGameSound('nav'); window.switchView('view-clash'); if(currentCRDeck.length === 0) window.generateSmartDeck(); };
    window.generateSmartDeck = () => {
        ...
        const tanks = crData.filter(c => c.role === 'tank' || c.role === 'mini_tank');
        const cycles = crData.filter(c => c.role === 'cycle' || c.e <= 2);
        let deck = [];
        let deckIds = new Set();
        let hasChampion = false;
        const addCard = (pool, fallbackPool = crData) => {
            let available = pool.filter(c => !deckIds.has(c.id) && !(hasChampion && c.r === 'champion'));
            if(available.length === 0) {
                available = fallbackPool.filter(c => !deckIds.has(c.id) && !(hasChampion && c.r === 'champion'));
            }
        ...
        addCard(bigSpells);
        addCard(antiAir);
        if(Math.random() > 0.5) addCard(buildings); else addCard(tanks);
        while(deck.length < 8) {
            let avg = deck.reduce((a,b)=>a+b.e,0) / deck.length;
            if(avg > 3.8) { addCard(cycles); } else { addCard(crData); }
        }
        renderDeck(deck);
    };
    ...
    function renderDeck(deck) {
        ...
        else if(deck.some(c => c.n === "Miner" || c.n === "Goblin Barrel")) type = "Kontrol / Bait";
        else if(deck.some(c => c.n === "X-Bow" || c.n === "Mortar")) type = "Kuşatma (Siege)";
    ...
    window.copyToCR = () => {
        if (!currentCRDeck || currentCRDeck.length !== 8) { return window.showToast("Önce bir deste oluşturulmalı!", "error"); }
        ...
        if (isMobile) { window.open(finalLink, '_blank'); } else { window.open(finalLink, '_blank'); }
    };
    ...
    window.renderAvatars = function(tid, iid, initialValue) {
        ...
        avatarList.forEach(i => {
            ...
            d.onclick=()=>{
                window.playGameSound('nav');
                c.querySelectorAll('.avatar-option').forEach(e=>e.classList.remove('selected'));
                d.classList.add('selected');
                document.getElementById(iid).value=i;
            };
            c.appendChild(d);
        });
        const selectedValue = initialValue && avatarList.includes(initialValue) ? initialValue : avatarList[0];
        markAvatarSelection(c, selectedValue);
        const input = document.getElementById(iid);
        if (input) input.value = selectedValue;
    };
    window.renderAvatars('authAvatarGrid','selectedAvatar');
    window.renderAvatars('settingsAvatarGrid', 'settingsSelectedAvatar');
    ...
    window.switchView = (id) => {
        ...
        if ((window.activeFullscreenBoardMode === '1v1' && id !== 'view-1v1-game') || (window.activeFullscreenBoardMode === '2v2' && id !== 'view-2v2-game')) {
            window.closeBoardFullscreen();
        }
        ...
        if (previousView === 'view-solo-training' && id !== 'view-solo-training' && window.stopSoloTrainingLiveEngine) {
            window.stopSoloTrainingLiveEngine();
        }
    ...
    onAuthStateChanged(auth, async u => {
        if(u){
            ...
            if (window.subscribeSocialCollections) window.subscribeSocialCollections();
            if (window.subscribeNotificationCollection) window.subscribeNotificationCollection();
            if (window.loadMyTournaments) window.loadMyTournaments();
            if (window.initIdrisListener) window.initIdrisListener();
            setTimeout(function() {
                if (window.processPendingDeepLink && window.processPendingDeepLink()) return;
                if (window.maybeOpenSharedAnalysisFromUrl && !window.maybeOpenSharedAnalysisFromUrl()) {
                    if (window.scheduleDashboardReconnectPrompt) window.scheduleDashboardReconnectPrompt();
                }
            }, 120);
        } else {
            window.stopDashboardListeners();
    ...
    window.removeTournament = async(tid) => {
        ...
        if(res.isConfirmed) {
            await updateDoc(doc(db,"tournaments",tid), { participantIds: arrayRemove(window.currentUser.uid) });
    ...
    document.getElementById('btnCreateTournament').onclick=async()=>{
        ...
        const n=document.getElementById('newTournamentName').value.trim(), c=parseInt(document.getElementById('playerCount').value);
        if(!n) return window.showToast("Lütfen bir turnuva adı girin.", "error");
        btn.disabled = true;
        btn.innerText = "OLUŞTURULUYOR...";
        const shortId = window.makeId(5);
        const slots=[];
        for(let i=0;i<c;i++) slots.push({index:i, name:`Masa ${i+1}`, ownerId:null, avatar:'fa-chair', status:'open'});
        await setDoc(doc(db,"tournaments",shortId),{ name:n, creatorId:window.currentUser.uid, status:'lobby', slots, matches:[], participantIds:[window.currentUser.uid], rules:"", createdAt:new Date() });
        btn.disabled = false;
    ...
    document.getElementById('btnJoinTournament').onclick=()=>{
        const id=document.getElementById('joinCodeInput').value.trim().toUpperCase();
        if(id && window.enterTournament) window.enterTournament(id);
    };

    window.saveSettings = async () => {
        const activeUser = auth.currentUser;
        if (!activeUser) return window.showToast('Oturum bulunamadı.', 'error');

        const btn = document.getElementById('btnSaveSettings');
        const nameInput = document.getElementById('settingsNameInput');
        const avatarInput = document.getElementById('settingsSelectedAvatar');
        const displayName = ((nameInput && nameInput.value) || '').trim();
        const avatar = ((avatarInput && avatarInput.value) || '').trim() || 'fa-chess-pawn';

        if (!displayName) return window.showToast('Profil ismi boş bırakılamaz.', 'error');
        ...
        try {
            ...
            await setDoc(doc(db, 'profiles', activeUser.uid), {
                uid: activeUser.uid,
                displayName: displayName,
                avatar: avatar,
                email: activeUser.email || '',
                lastView: window.currentViewId || 'view-settings'
            }, { merge: true });

            window.currentUser = auth.currentUser;
            document.getElementById('currentUserDisplay').innerText = displayName;
            document.getElementById('currentUserIcon').innerHTML = `<i class="fas ${avatar}" style="color:var(--primary)"></i>`;
            if (window.ensureUserProfile) await window.ensureUserProfile();
            if (window.pushProfilePresence) await window.pushProfilePresence();
            syncSettingsFormFromCurrentUser();
            window.showToast('Profil ayarları kaydedildi.', 'success');
            window.switchView('view-dashboard');
        } catch (err) {
            console.error(err);
            window.showToast(err.message || 'Ayarlar kaydedilemedi.', 'error');
        } finally {
            if (btn) {
                btn.disabled = false;
    ...
    cb.onclick=()=>{ cw.style.display='flex'; cb.style.display='none'; document.getElementById('chatBadge').style.display='none'; if (window.scrollChat) window.scrollChat(); };
    window.hideChat=()=>{ cw.style.display='none'; if(window.currentTournamentId || window.current2v2Id || window.current1v1Id) cb.style.display='flex'; };
    ...
    document.getElementById('chatInputArea').onsubmit=async(e)=>{
        ...
        if(!text) return;
        if(!window.throttleAction('room_chat', window.activeChatThreadId || 'default', 5, 12000)) return window.showToast('Sohbet limiti doldu. Biraz bekle.', 'error');
        const id = window.activeChatThreadId || window.currentTournamentId || window.current2v2Id || window.current1v1Id;
        await addDoc(collection(db,`tournaments/${id}/messages`),{text:text, user:window.currentUser.displayName, uid:window.currentUser.uid, createdAt:serverTimestamp()});
        i.value='';
    };
