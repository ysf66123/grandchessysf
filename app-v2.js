(async function () {
  ...
  window.getActivePieceSet = function () {
    const owned =
      ...
      Array.isArray(window.currentProfileData.storyOwnedPieceSets)
        ? window.currentProfileData.storyOwnedPieceSets
        : ["neo"];
    const active =
      window.currentProfileData && window.currentProfileData.activePieceSet
        ? window.currentProfileData.activePieceSet
        : "neo";
    const set = window.STORY_PIECE_SETS.find(function (item) {
      return item.id === active && owned.indexOf(item.id) !== -1;
    });
  ...
  window.getPieceAsset = function (color, type) {
    ...
    const pieceType =
      ["p", "n", "b", "r", "q", "k"].indexOf(type) !== -1 ? type : "p";
    const activeSet = window.getActivePieceSet();
    return (
      "https://images.chesscomfiles.com/chess-themes/pieces/" +
      activeSet.theme +
      "/150/" +
      pieceColor +
      pieceType +
  ...
  const soundGenerators = {
    ...
    promotion: function () {
      ...
      setTimeout(function () {
        playTone(880, 0.11, "triangle", 0.16);
      }, 150);
    },
    checkmate: function () {
      ...
      setTimeout(function () {
        playTone(554, 0.16, "square", 0.14);
      }, 120);
      setTimeout(function () {
        playTone(370, 0.2, "sawtooth", 0.12);
      }, 270);
    },
    notify: function () {
    ...
    quizWrong: function () {
      ...
      setTimeout(function () {
        playTone(250, 0.25, "sawtooth", 0.25);
      }, 150);
    },
  };
  ...
  window.playChessMoveSound = function (move, game) {
    ...
    const isMate =
      game && typeof game.in_checkmate === "function" && game.in_checkmate();
    const isCheck =
      game && typeof game.in_check === "function" && game.in_check();
    const isCapture =
      !!move.captured || flags.indexOf("c") !== -1 || flags.indexOf("e") !== -1;
  ...
  window.playChessMoveSoundFromFlags = function (flags, game, isCheckFlag) {
    ...
    const isCheck =
      ...
      (game && typeof game.in_check === "function" && game.in_check());
    const isCapture =
      moveFlags.indexOf("c") !== -1 || moveFlags.indexOf("e") !== -1;
    const isCastle =
      moveFlags.indexOf("k") !== -1 || moveFlags.indexOf("q") !== -1;
  ...
  function syncSettingsFormFromCurrentUser() {
    ...
    if (muteToggle)
      muteToggle.checked = localStorage.getItem("gm_mute") !== "true";
    const langSelect = document.getElementById("settingsAnalysisLang");
    if (langSelect)
      langSelect.value = localStorage.getItem("gm_analysis_lang") || "tr";
    ...
    if (dashSelect) {
      ...
      const defaultLayout = isMobile ? "modern" : "classic";
      dashSelect.value =
        localStorage.getItem("gm_dashboard_layout") || defaultLayout;
    }
  ...
  window.GM_APP_BASE_URL = window.GM_APP_BASE_URL || "http://localhost:3000";
  ...
  window.renderAvatars = function (tid, iid, initialValue) {
    ...
    const selectedValue =
      initialValue && avatarList.includes(initialValue)
        ? initialValue
        : avatarList[0];
  ...
  window.switchView = (id) => {
    ...
    if (
      previousView === "view-solo-training" &&
      id !== "view-solo-training" &&
  ...
  onAuthStateChanged(auth, async (u) => {
    if (u) {
      ...
      setTimeout(function () {
        if (window.processPendingDeepLink && window.processPendingDeepLink())
          return;
        if (
  ...
  window.loadMyTournaments = () => {
    ...
    onSnapshot(q, (snap) => {
      ...
      if (!l) return;
      if (l) l.innerHTML = "";
      if (typeof mL !== "undefined" && mL) mL.innerHTML = "";
      if (snap.empty)
        l.innerHTML =
  ...
  document.getElementById("btnCreateTournament").onclick = async () => {
    ...
    const targetN =
      document.getElementById("modern_newTournamentName") &&
      document.getElementById("dashboard-modern-layout").style.display !==
        "none"
        ? document.getElementById("modern_newTournamentName")
        : document.getElementById("newTournamentName");
    const targetC =
      document.getElementById("modern_playerCount") &&
      document.getElementById("dashboard-modern-layout").style.display !==
        "none"
        ? document.getElementById("modern_playerCount")
  ...
  window.saveSettings = async () => {
    ...
    const avatar =
      ((avatarInput && avatarInput.value) || "").trim() || "fa-chess-pawn";

    if (!displayName)
      return window.showToast("Profil ismi boş bırakılamaz.", "error");

    if (btn) {
      if (btn) btn.disabled = true;
      if (typeof mBtn !== "undefined" && mBtn) mBtn.disabled = true;
      if (btn) btn.innerText = "KAYDEDİLİYOR...";
      if (typeof mBtn !== "undefined" && mBtn)
        mBtn.innerText = "KAYDEDİLİYOR...";
    }
    ...
    try {
      ...
      if (langSelect)
        localStorage.setItem(
          "gm_analysis_lang",
          langSelect.value === "en" ? "en" : "tr",
        );
      const dashSelect = document.getElementById("settingsDashboardLayout");
      if (dashSelect)
        localStorage.setItem("gm_dashboard_layout", dashSelect.value);
      window.applyDashboardLayout();

      await setDoc(
        doc(db, "profiles", activeUser.uid),
        {
          uid: activeUser.uid,
          displayName: displayName,
          avatar: avatar,
      ...
      document.getElementById("currentUserIcon").innerHTML =
        `<i class="fas ${avatar}" style="color:var(--primary)"></i>`;
      if (window.ensureUserProfile) await window.ensureUserProfile();
      ...
      if (btn) {
        ...
        if (typeof mBtn !== "undefined" && mBtn) mBtn.innerText = "KAYDET";
      }
    }
  };
          ...
          indicators.forEach((ind, idx) => {
            ...
            if (idx === activeIndex) {
              ind.style.background =
                activeCard.style.getPropertyValue("--card-color") || "#fff";
            } else {
...
window.applyDashboardLayout = function () {
  ...
  const defaultLayout = isMobile ? "modern" : "classic";
  const layout = localStorage.getItem("gm_dashboard_layout") || defaultLayout;
