...
// Replace everything from `window.activeQuizBuilderIndex =` to the end of `updateBuilderUI`
const replacement = `function updateBuilderUI() {
    ...
    quizBuilderQuestions.forEach((q, idx) => {
        ...
        div.style.marginBottom = '30px'; // Space between cards
...
code = code.replace(/window\.activeQuizBuilderIndex[\s\S]+?\}\s*const sum = document\.getElementById\('builderSummary'\);\s*if \(sum\) sum\.innerText = \`\$\{quizBuilderQuestions\.length\}\`;\s*\}/, replacement);
