...
function assign1v1HostColor(players, hostId, preference) {
    ...
    if (preference === 'random') preference = Math.random() < 0.5 ? 'white' : 'black';
    var hostPlayer = players.find(function(player) { return player.uid === hostId; });
    if (!hostPlayer) return players;
    var otherTeam = preference === 'white' ? 'black' : 'white';
    return players.map(function(player) {
        if (player.uid === hostId) return Object.assign({}, player, { team: preference });
        if (player.team === preference) return Object.assign({}, player, { team: otherTeam });
        return player;
    }).sort(function(a, b) {
...
let soloTrainingState = {
    ...
    evalText: '--',
    best: {
        white: { fen: null, uci: null, san: '-', evalText: '--', statusText: 'Motor bekleniyor' },
        black: { fen: null, uci: null, san: '-', evalText: '--', statusText: 'Motor bekleniyor' }
    },
    brilliant: {
        stateText: 'Bekleniyor',
        white: { fen: null, uci: null, san: '-', note: 'Aday aranıyor', found: false },
        black: { fen: null, uci: null, san: '-', note: 'Aday aranıyor', found: false }
    },
    mistakes: {
        stateText: 'Bekleniyor',
        white: {},
...
Object.defineProperties(window, {
    ...
    chess1v1: { get: () => chess1v1, set: (v) => { chess1v1 = v; } },
    ...
    soloTrainingChess: { get: () => soloTrainingChess, set: (v) => { soloTrainingChess = v; } }
});
...
function buildQuickMatch1v1GameData(code, whitePlayer, blackPlayer, timeControl) {
    ...
    return {
        ...
        moveCount: 0,
        players: [
            { uid: whitePlayer.uid, name: whitePlayer.name, avatar: whitePlayer.avatar || 'fa-chess-pawn', team: 'white', index: 0, isReady: true },
            { uid: blackPlayer.uid, name: blackPlayer.name, avatar: blackPlayer.avatar || 'fa-chess-pawn', team: 'black', index: 0, isReady: true }
        ],
...
async function finishQuickMatch1v1(gameCode) {
    ...
    if (window.currentUser) {
        deleteDoc(doc(db, 'quick_match_1v1', window.currentUser.uid)).catch(function() {});
    }
...
async function tryResolveQuickMatch1v1() {
    ...
    const now = Date.now();
    const staleMs = 90000;
    const snapshot = await getDocs(query(collection(db, 'quick_match_1v1'), where('status', '==', 'waiting'))).catch(function() { return null; });
    ...
    try {
        await runTransaction(db, async function(transaction) {
            ...
            if (!mySnap.exists() || !opponentSnap.exists()) throw new Error('queue_changed');
            const myData = mySnap.data() || {};
            const opponentData = opponentSnap.data() || {};
            if (myData.status !== 'waiting' || opponentData.status !== 'waiting') throw new Error('queue_changed');
            if (myData.uid === opponentData.uid || myData.uid !== myUid) throw new Error('queue_changed');
            if ((Date.now() - (opponentData.updatedAtMs || opponentData.createdAtMs || 0)) > staleMs) throw new Error('queue_stale');
            ...
            transaction.set(gameRef, buildQuickMatch1v1GameData(code, white, black, myData.timeControl || opponentData.timeControl || 5));
            transaction.set(myRef, { status: 'matched', gameCode: code, matchedUid: opponentData.uid, updatedAtMs: Date.now() }, { merge: true });
            transaction.set(opponentRef, { status: 'matched', gameCode: code, matchedUid: myUid, updatedAtMs: Date.now() }, { merge: true });
            matchedCode = code;
        });
    } catch (e) {
...
window.cancelQuickMatch1v1 = async function() {
    ...
    clearQuickMatch1v1Local();
    if (window.currentUser) {
        await deleteDoc(doc(db, 'quick_match_1v1', window.currentUser.uid)).catch(function() {});
    }
    if (wasActive) window.showToast('Hizli eslesme iptal edildi.', 'info');
};

window.startQuickMatch1v1 = async function() {
    if (!window.currentUser) return window.showToast('Once giris yapmalisin.', 'error');
    if (quickMatch1v1Active) return window.showToast('Zaten eslesme araniyor.', 'info');
    ...
    quickMatch1v1Unsubscribe = onSnapshot(myRef, function(snap) {
        ...
        if (data.status === 'matched' && data.gameCode) finishQuickMatch1v1(data.gameCode);
    });
    ...
    Swal.fire({
        ...
        if (result.dismiss === Swal.DismissReason.cancel) window.cancelQuickMatch1v1();
    });
...
function buildBotCandidateLines(result, config) {
    ...
    if (!lines.length && result && result.bestMove) {
        lines.push({ rank: 1, uci: result.bestMove, cp: result.cp, mate: result.mate });
    }
    ...
    return lines.filter(function(line, index) {
        ...
        if (best.mate !== null && best.mate !== undefined) {
            if (best.mate > 0 && line.mate !== null && line.mate !== undefined) return line.mate > 0;
            return index === 0;
        }
        if (best.cp !== null && best.cp !== undefined && line.cp !== null && line.cp !== undefined) {
...
function pick1v1BotMoveUci(result, level) {
    ...
    var weights = candidates.map(function(candidate, index) {
        ...
        return (remainingWeight / Math.max(1, candidates.length - 1)) * Math.pow(0.72, index - 1);
    });
...
function evaluate1v1Position(game) {
    ...
    var pieceValues = { p: 100, n: 320, b: 330, r: 500, q: 900, k: 0 };
    var centerSquares = { d4: 1, e4: 1, d5: 1, e5: 1, c3: 0.5, f3: 0.5, c6: 0.5, f6: 0.5, c4: 0.5, f4: 0.5, c5: 0.5, f5: 0.5 };
    var score = 0;
    ...
    for (var rank = 0; rank < 8; rank++) {
        for (var file = 0; file < 8; file++) {
            ...
            if (piece.type === 'p' && (square[1] === '4' || square[1] === '5')) developmentBonus += 8;
...
function pick1v1FallbackMove(fen, level) {
    ...
    var moves = probe.moves({ verbose: true }).map(function(move) {
        probe.move({ from: move.from, to: move.to, promotion: move.promotion || 'q' });
        var searchScore = -search1v1BotScore(probe, Math.max(0, searchDepth - 1), -Infinity, Infinity);
        probe.undo();
        return {
            move: move,
    ...
    var pickWindow = Math.max(1, Math.min(moves.length, config.fallbackPickCount || 1));
    var pickIndex = 0;
...
function maybeSchedule1v1BotMove(data) {
    ...
    pending1v1BotMoveTimer = setTimeout(async function() {
        ...
        try {
            ...
            var latest = current1v1Data;
            if (!latest || latest.status !== 'active' || latest.moveCount !== data.moveCount || latest.hostId !== currentUser.uid) return;
...
function render1v1Lobby(data) {
    ...
    if (isHost) {
        document.getElementById('timeControlSelect1v1').value = data.timeControl || 5;
        var colorSelect = document.getElementById('hostColorSelect1v1');
        if (colorSelect) colorSelect.value = normalize1v1HostColorPreference(data.hostColorPreference);
    }
        ...
        if (isBotPlayer(player)) {
            ...
            el.innerHTML = '<div class="seat-shell">'
                + '<div class="seat-main">'
                    ...
                    + '<div class="seat-sub">Motor derinliği: ' + botConfig.depth + '. seviye</div>'
                + '</div>'
            ...
            el.innerHTML = '<div class="seat-shell">'
                + '<div class="seat-main">'
                    ...
                    + '<div class="seat-sub">' + (player.uid === window.currentUser.uid ? 'Bu koltuk sende.' : 'Oyuncu hazırlık bekliyor.') + '</div>'
                + '</div>'
            + '</div>';
        } else {
            el.innerHTML = '<div class="seat-shell">'
...
window.add1v1Bot = async function(team, level) {
    ...
    if (targetSeat.uid && !isBotPlayer(targetSeat)) return window.showToast('Bu taraf zaten bir oyuncu tarafından alındı.', 'error');
    await updateDoc(doc(db, 'games_1v1', current1v1Id), {
...
document.getElementById('hostColorSelect1v1').onchange = async function() {
    if (!current1v1Id || !current1v1Data || current1v1Data.hostId !== window.currentUser.uid) return;
    var preference = normalize1v1HostColorPreference(document.getElementById('hostColorSelect1v1').value);
    var updates = { hostColorPreference: preference };
    if (preference !== 'random') {
        updates.players = assign1v1HostColor(current1v1Data.players, current1v1Data.hostId, preference);
    }
    await updateDoc(doc(db, 'games_1v1', current1v1Id), updates);
};
...
function renderLiveMovePanel(options) {
    ...
    if (latestEl) latestEl.innerText = history.length ? history[history.length - 1].san : '-';
    if (statusEl) statusEl.innerText = options.statusText || 'Canli oyun';

    if (!history.length) {
        listEl.innerHTML = '<div class="live-move-empty">Hamleler burada akacak.<br>Oyun basladiginda liste otomatik dolacak.</div>';
        return 0;
    }

    var rows = buildLiveMoveHistoryRows(history);
    listEl.innerHTML = rows.map(function(row) {
        return '<div class="live-move-row' + (row.isLatest ? ' is-latest' : '') + '">'
            + '<span class="live-move-number">' + row.number + '.</span>'
            + '<span class="live-move-san">' + window.escapeHtml(row.white || '...') + '</span>'
            + '<span class="live-move-san' + (row.black ? '' : ' empty') + '">' + window.escapeHtml(row.black || '...') + '</span>'
            + '</div>';
    }).join('');
...
function isTrainingAssistAdmin() {
    var email = window.currentUser && window.currentUser.email ? String(window.currentUser.email).toLowerCase() : '';
    return email === TRAINING_ASSIST_ADMIN_EMAIL;
}

function is1v1BotTrainingMode(data) {
    return !!(data && Array.isArray(data.players) && data.players.some(function(player) { return isBotPlayer(player); }));
}
...
function get1v1TrainingPlayer(data) {
    ...
    return data.players.find(function(player) { return player.uid === window.currentUser.uid; }) || null;
}
...
function sync1v1TrainingAssistUI(data) {
    ...
    if (!card || !contextEl || !bestEl || !evalEl || !statusEl || !playBtn || !autoBtn) return;

    var visible = isTrainingAssistAdmin() && is1v1BotTrainingMode(data) && current1v1Role === 'player';
    card.style.display = visible ? 'grid' : 'none';
    if (!visible) return;

    var isMyTurn = is1v1TrainingUserTurn(data);
    contextEl.innerText = data.status !== 'active'
        ? 'Mac aktif degil'
        : (isMyTurn ? 'Sira sende' : 'Rakip oynuyor');
    ...
    playBtn.disabled = !isMyTurn || !trainingAssist1v1State.bestMoveUci || data.status !== 'active';
    autoBtn.innerHTML = '<i class="fas fa-wand-magic-sparkles"></i> ' + (trainingAssist1v1State.autoPlay ? 'Oto Acik' : 'Oto Kapali');
...
async function playTrainingAssistMove1v1Internal() {
    ...
    if (!bestUci || trainingAssist1v1State.fen !== fen || trainingAssist1v1State.applyingFen === fen) return false;
...
async function refresh1v1TrainingAssist(data, force) {
    ...
    if (!isTrainingAssistAdmin() || !is1v1BotTrainingMode(data) || current1v1Role !== 'player') return;
    if (!data || data.status !== 'active') {
        trainingAssist1v1State.statusText = 'Panel sadece aktif bot macinda kullanilir.';
        sync1v1TrainingAssistUI(data);
        return;
    }
    ...
    var fen = chess1v1.fen();
    if (!force && trainingAssist1v1State.fen === fen && trainingAssist1v1State.bestMoveUci) {
        sync1v1TrainingAssistUI(data);
        if (trainingAssist1v1State.autoPlay) await playTrainingAssistMove1v1Internal();
        return;
    }
    ...
    try {
        ...
        var bestUci = result && (result.bestMove || (result.topLines && result.topLines[0] && result.topLines[0].uci));
        ...
        sync1v1TrainingAssistUI(data);
        if (trainingAssist1v1State.autoPlay && bestUci) await playTrainingAssistMove1v1Internal();
    } catch (e) {
        if (token !== trainingAssist1v1State.requestToken) return;
        trainingAssist1v1State.statusText = 'Motor sorgusu basarisiz oldu. Tekrar dene.';
        sync1v1TrainingAssistUI(data);
    }
}
...
window.toggleTrainingAssistAuto1v1 = function() {
    ...
    if (trainingAssist1v1State.autoPlay) refresh1v1TrainingAssist(current1v1Data, false);
};
...
function syncSoloTrainingAccess() {
    ...
    if (window.currentViewId === 'view-solo-training' && !isSoloTrainingAllowed()) {
...
function formatSoloMoveText(san) {
    if (!san || san === '-') return '-';
    return soloTrainingState.moveLanguage === 'tr' ? translateSanToTurkish(san) : san;
}
...
function getSoloGameStatusText() {
    if (typeof soloTrainingChess.in_checkmate === 'function' && soloTrainingChess.in_checkmate()) {
        return getSoloColorName(soloTrainingChess.turn() === 'w' ? 'black' : 'white') + ' mat verdi';
    }
    if (typeof soloTrainingChess.in_stalemate === 'function' && soloTrainingChess.in_stalemate()) return 'Pat';
    if (typeof soloTrainingChess.in_draw === 'function' && soloTrainingChess.in_draw()) return 'Beraberlik';
    if (typeof soloTrainingChess.in_check === 'function' && soloTrainingChess.in_check()) {
...
function isSoloGameOver() {
    return typeof soloTrainingChess.game_over === 'function' && soloTrainingChess.game_over();
}
...
function isSoloDrawishAfterMove(fen, uci) {
    ...
    try {
        ...
        if (typeof probe.in_checkmate === 'function' && probe.in_checkmate()) return false;
        if (typeof probe.in_stalemate === 'function' && probe.in_stalemate()) return true;
        if (typeof probe.in_threefold_repetition === 'function' && probe.in_threefold_repetition()) return true;
...
function pickSoloBestLineAvoidingDraw(fen, result) {
    ...
    if (sideEval === null || sideEval < 180) return { line: fallback, avoidedDraw: false };
    if (!isSoloDrawishAfterMove(fen, fallback.uci)) return { line: fallback, avoidedDraw: false };

    var bestCp = fallback.cp !== null && fallback.cp !== undefined ? fallback.cp : result.cp;
    for (var i = 1; i < lines.length; i++) {
        var line = lines[i];
        if (!line || !line.uci || isSoloDrawishAfterMove(fen, line.uci)) continue;
        if (result.mate !== null && result.mate !== undefined) return { line: line, avoidedDraw: true };
        if (bestCp === null || bestCp === undefined || line.cp === null || line.cp === undefined || (bestCp - line.cp) <= 140) {
    ...
    return {
        ...
        statusText: bestUci
            ? (selected.avoidedDraw
                ...
                : (result && result.fallback ? 'Materyal hesabi; motor hatti isinmadi.' : 'Tikla ve tahtada kisa animasyonla gor.'))
...
function getSoloMoveCategoryLabel(category) {
    ...
    return (soloTrainingState.moveLanguage === 'tr' ? tr : en)[category] || category;
}
...
function classifySoloPlayedMove(move, side, beforeFen) {
    ...
    if (best.uci === uci || (best.uci && best.uci.slice(0, 4) === uci.slice(0, 4) && (!best.uci[4] || best.uci[4] === uci[4]))) return 'best';
    if (move.san && move.san.indexOf('#') !== -1) return 'brilliant';
    if (move.flags && (move.flags.indexOf('k') !== -1 || move.flags.indexOf('q') !== -1)) return 'excellent';
    ...
    if (best.uci && uci && best.uci.slice(0, 4) !== uci.slice(0, 4)) return 'inaccuracy';
    return 'good';
}

function recordSoloMoveClassification(move, side, beforeFen, forcedCategory) {
    var category = forcedCategory || classifySoloPlayedMove(move, side, beforeFen);
...
function getSoloMoveBaseUci(moveOrUci) {
    var uci = typeof moveOrUci === 'string' ? moveOrUci : getSoloMoveUci(moveOrUci);
...
function getSoloHangingLossAfterMove(afterChess, move, beforePiece, capturedValue) {
    ...
    attackers.forEach(function(reply) {
        ...
        if (attacker) cheapestAttacker = Math.min(cheapestAttacker, getSoloPieceValue(attacker.type) * 100);
    });
    if (cheapestAttacker === 900) cheapestAttacker = 100;
    return Math.max(0, movedValue - capturedValue - Math.max(0, cheapestAttacker - 100));
}
...
function classifySoloCandidateRisk(move, beforeFen, result, topBases) {
    ...
    var capturedValue = move.captured ? getSoloPieceValue(move.captured) * 100 : 0;
    var movedValue = getSoloPieceValue(beforePiece.type) * 100;
    ...
    var hangingLoss = getSoloHangingLossAfterMove(after, played, beforePiece, capturedValue);
    var badCaptureLoss = move.captured && movedValue >= capturedValue + 400 ? movedValue - capturedValue : 0;
    ...
    if (risk >= 650 || (movedValue >= 900 && hangingLoss >= 500)) {
        return { category: 'blunder', risk: risk, reason: 'Agir materyal kaybi riski' };
    }
    if (risk >= 320 || (bestSideEval !== null && bestSideEval >= 180 && !move.captured && movedValue >= 500 && hangingLoss >= 250)) {
        return { category: 'mistake', risk: risk, reason: 'Avantaji veya materyali ciddi azaltir' };
    }
    if (risk >= 90 || (bestSideEval !== null && bestSideEval >= 120) || !move.captured) {
        return { category: 'inaccuracy', risk: Math.max(risk, 40), reason: 'Motorun ana hattindan uzaklasir' };
    }
    return null;
}
    ...
    try {
        ...
        legalMoves.forEach(function(move) {
            ...
            buckets[riskInfo.category].push({
                ...
                san: move.san,
                note: riskInfo.reason + ' (~' + (riskInfo.risk / 100).toFixed(1) + ' piyon)',
                found: true,
                risk: riskInfo.risk
            });
        });
        Object.keys(buckets).forEach(function(category) {
            buckets[category].sort(function(a, b) { return b.risk - a.risk; });
            set[category] = buckets[category][0] || emptySoloMistakeCandidate('Bu seviyede riskli hamle yok');
        });
        return set;
    } catch (e) {
...
function setSoloMistakeCard(side, category, data) {
    ...
    var moveEl = document.getElementById('soloMistake' + sideTitle + categoryTitle);
    var noteEl = document.getElementById('soloMistake' + sideTitle + categoryTitle + 'Note');
    ...
    if (noteEl) noteEl.innerText = candidate.note || 'Bu seviyede riskli hamle yok';
    if (card) {
...
function syncSoloMistakePanel() {
    ...
    if (statusEl) statusEl.innerText = soloTrainingState.mistakes.stateText || 'Bekleniyor';
...
function detectSoloBrilliantMove(color, fen, result) {
    var bestUci = result && (result.bestMove || (result.topLines && result.topLines[0] && result.topLines[0].uci));
    if (!fen || !bestUci || bestUci.length < 4) return emptySoloBrilliant('Motor aday bulamadi');
    try {
        var probe = new Chess();
        if (!probe.load(fen)) return emptySoloBrilliant('Konum bu taraf icin uygun degil');
        ...
        var isMateShot = result.mate !== null && result.mate !== undefined && result.mate > 0 && Math.abs(result.mate) <= 5;
        var isMajorTactic = sideEval !== null && sideEval >= 180 && (
            move.san.indexOf('+') !== -1 ||
            move.san.indexOf('#') !== -1 ||
            !!move.promotion ||
            (move.captured && movedValue >= capturedValue + 2)
        );
        var isQuietBrilliant = sideEval !== null && sideEval >= 260 && !move.captured && movedValue >= 3 && move.san.indexOf('+') !== -1;
        ...
        var reason = isMateShot
            ...
            : (move.captured && movedValue >= capturedValue + 2 ? 'Motor avantajli feda fikri buldu' : 'Yuksek etkili taktik hamle');
        return {
            fen: fen,
            uci: bestUci,
...
function syncSoloAutoControls() {
    ...
    ['none', 'white', 'black', 'both'].forEach(function(key) {
        var el = document.getElementById('soloAutoSide' + key.charAt(0).toUpperCase() + key.slice(1));
        if (el) el.classList.toggle('active', side === key);
    });
    var delayInput = document.getElementById('soloAutoDelayInput');
    if (delayInput && Number(delayInput.value) !== soloTrainingState.autoDelaySec) {
    ...
    if (enBtn) enBtn.classList.toggle('active', soloTrainingState.moveLanguage === 'en');
    if (trBtn) trBtn.classList.toggle('active', soloTrainingState.moveLanguage === 'tr');
}
...
async function playSoloTrainingBestMoveInternal(color, expectedFen) {
    ...
    if (!move) return false;
    recordSoloMoveClassification(move, color, beforeFen, best.uci === (best.uci.slice(0, 4) + (move.promotion || '')) ? 'best' : null);
    soloTrainingSelectedSquare = null;
    soloTrainingValidMoves = [];
    clearSoloPreviewMarks();
    drawSoloTrainingBoard();
    if (window.playChessMoveSound) window.playChessMoveSound(move, soloTrainingChess);
...
function scheduleSoloAutoMove() {
    ...
    if (soloTrainingState.sync.live) {
        setSoloAutoStatus('Canli mac aktarimi aktifken otomatik oynatma beklemede.');
        return;
    }
    ...
    var waitSec = Math.max(1, Math.min(60, parseInt(soloTrainingState.autoDelaySec, 10) || 5));
    ...
    setSoloAutoStatus(sideLabel + ' sirasi: ' + waitSec + ' sn hamle gelmezse ' + formatSoloMoveText(best.san) + ' oynanacak.');
...
function syncSoloTrainingUI() {
    ...
    var classificationToggle = document.getElementById('soloClassificationToggle');
    ...
    if (engineEl) engineEl.innerText = soloTrainingState.engineState || 'Bekleniyor';
    if (evalEl) evalEl.innerText = soloTrainingState.evalText || '--';
    if (statusEl) statusEl.innerText = soloTrainingState.statusText || 'Hamle oynadiginda oneriler otomatik yenilenir.';
    if (whiteMoveEl) whiteMoveEl.innerText = formatSoloMoveText(soloTrainingState.best.white.san);
    if (blackMoveEl) blackMoveEl.innerText = formatSoloMoveText(soloTrainingState.best.black.san);
    if (whiteEvalEl) whiteEvalEl.innerText = soloTrainingState.best.white.evalText + ' - ' + soloTrainingState.best.white.statusText;
    if (blackEvalEl) blackEvalEl.innerText = soloTrainingState.best.black.evalText + ' - ' + soloTrainingState.best.black.statusText;
    ...
    if (moveCountEl) moveCountEl.innerText = String(soloTrainingChess.history().length);
    if (whiteTag) whiteTag.classList.toggle('active', turnColor === 'white' && !isSoloGameOver());
    if (blackTag) blackTag.classList.toggle('active', turnColor === 'black' && !isSoloGameOver());
    if (brilliantStateEl) brilliantStateEl.innerText = soloTrainingState.brilliant.stateText || 'Bekleniyor';
    if (brilliantWhiteMoveEl) brilliantWhiteMoveEl.innerText = formatSoloMoveText(soloTrainingState.brilliant.white.san);
    if (brilliantBlackMoveEl) brilliantBlackMoveEl.innerText = formatSoloMoveText(soloTrainingState.brilliant.black.san);
    if (brilliantWhiteNoteEl) brilliantWhiteNoteEl.innerText = soloTrainingState.brilliant.white.note || 'Aday aranıyor';
    if (brilliantBlackNoteEl) brilliantBlackNoteEl.innerText = soloTrainingState.brilliant.black.note || 'Aday aranıyor';
    if (brilliantWhiteCard) {
        brilliantWhiteCard.disabled = !soloTrainingState.brilliant.white.found;
        brilliantWhiteCard.classList.toggle('found', !!soloTrainingState.brilliant.white.found);
    }
    if (brilliantBlackCard) {
        brilliantBlackCard.disabled = !soloTrainingState.brilliant.black.found;
        brilliantBlackCard.classList.toggle('found', !!soloTrainingState.brilliant.black.found);
    }
    ...
    if (classificationToggle) {
        classificationToggle.classList.toggle('active', !!soloTrainingState.showClassifications);
        classificationToggle.innerHTML = '<i class="fas fa-icons"></i> ' + (soloTrainingState.showClassifications ? 'Açık' : 'Kapalı');
    }

    if (bestPanelToggle) {
        bestPanelToggle.classList.toggle('active', !!soloTrainingState.showBestPanel);
        bestPanelToggle.innerHTML = '<i class="fas ' + (soloTrainingState.showBestPanel ? 'fa-eye' : 'fa-eye-slash') + '"></i> ' + (soloTrainingState.showBestPanel ? 'Acik' : 'Kapali');
    }
    if (bestPanelCard) bestPanelCard.style.display = soloTrainingState.showBestPanel ? '' : 'none';
    if (voiceToggle) {
        voiceToggle.classList.toggle('active', !!soloTrainingState.voiceEnabled);
        voiceToggle.innerHTML = '<i class="fas ' + (soloTrainingState.voiceEnabled ? 'fa-microphone-lines' : 'fa-microphone') + '"></i> ' + (soloTrainingState.voiceEnabled ? 'Acik' : 'Kapali');
    }
    if (voiceStatusEl) voiceStatusEl.innerText = soloTrainingState.voiceStatusText || 'Sesli hamle kapali.';
    if (voiceDeviceSelect && voiceDeviceSelect.value !== (soloTrainingState.voiceSettings.deviceId || '')) voiceDeviceSelect.value = soloTrainingState.voiceSettings.deviceId || '';
    if (voiceTurnEl) voiceTurnEl.innerText = getSoloColorName(turnColor);
    if (voiceEngineEl) {
        voiceEngineEl.innerText = soloTrainingState.voiceEngineText || 'Hazir degil';
        voiceEngineEl.classList.toggle('active', !!soloTrainingState.voiceEnabled);
    }
    if (voiceLastHeardEl) voiceLastHeardEl.innerText = soloTrainingState.lastHeardText || '-';
    if (cloudSttStatusEl) {
        cloudSttStatusEl.innerText = soloTrainingState.cloudSttStatusText || 'Kontrol ediliyor';
        cloudSttStatusEl.classList.toggle('ready', !!soloTrainingState.cloudSttConfigured);
    ...
    if (syncSourceEl) syncSourceEl.innerText = soloTrainingState.sync.sourceLabel || 'Bagli degil';
    if (syncMoveCountEl) syncMoveCountEl.innerText = String(soloTrainingState.sync.live ? soloTrainingChess.history().length : (soloTrainingState.sync.importedPly || 0));
    if (syncStatusEl) syncStatusEl.innerText = soloTrainingState.sync.statusText || 'Chrome eklentisini bu sekmede hedef yap.';
...
function renderSoloTrainingMoveHistory() {
    ...
    if (!history.length) {
        listEl.innerHTML = '<div class="live-move-empty">Ilk hamleni oynadiginda solo analiz defteri burada baslar.</div>';
        return;
    }
    var rows = buildLiveMoveHistoryRows(history);
    listEl.innerHTML = rows.map(function(row) {
        return '<div class="live-move-row' + (row.isLatest ? ' is-latest' : '') + '">'
            + '<span class="live-move-number">' + row.number + '.</span>'
            + '<span class="live-move-san">' + window.escapeHtml(row.white ? formatSoloMoveText(row.white) : '...') + '</span>'
            + '<span class="live-move-san' + (row.black ? '' : ' empty') + '">' + window.escapeHtml(row.black ? formatSoloMoveText(row.black) : '...') + '</span>'
            + '</div>';
    }).join('');
    listEl.scrollTop = listEl.scrollHeight;
}
...
function renderSoloTrainingBoardInto(boardEl) {
    ...
    if (soloTrainingState.showClassifications) {
        soloTrainingMoveClassifications.slice(0, soloTrainingChess.history().length).forEach(function(item) {
    ...
    for (var r = 0; r < 8; r++) {
        for (var c = 0; c < 8; c++) {
            ...
            if (soloTrainingSelectedSquare === squareName) div.classList.add('selected');
            if (soloTrainingValidMoves.indexOf(squareName) !== -1) div.classList.add('highlight');
            ...
            div.onclick = function(square) {
                return function() { handleSoloTrainingSquareClick(square); };
            }(squareName);

            if (sq) {
                var piece = document.createElement('div');
                piece.className = 'piece ' + (sq.color === activeColor && !isSoloGameOver() ? 'active' : 'locked');
                ...
                if (badgeInfo) {
                    ...
                    badge.innerHTML = '<i class="fas ' + getSoloMoveCategoryIconClass(badgeInfo.category) + '"></i>';
                    badge.title = getSoloMoveCategoryLabel(badgeInfo.category) + ' - ' + (badgeInfo.san || '');
...
function resetSoloEngineState(message) {
    ...
    soloTrainingState.statusText = 'Hamle oynadiginda oneriler otomatik yenilenir.';
...
async function refreshSoloBrilliantEngine(baseFen, whiteFen, blackFen) {
    if (!isSoloTrainingAllowed() || !window.queueStockfishEval || !window.initStockfish || isSoloGameOver()) return;
    var token = ++soloTrainingState.brilliantRequestToken;
    soloTrainingState.brilliant.stateText = 'Taranıyor';
    soloTrainingState.brilliant.white = emptySoloBrilliant('Beyaz icin taktik taraniyor');
    soloTrainingState.brilliant.black = emptySoloBrilliant('Siyah icin taktik taraniyor');
    syncSoloTrainingUI();
    try {
        await window.initStockfish();
        var whiteTask = whiteFen
            ? window.queueStockfishEval(whiteFen, { depth: Math.max(14, SOLO_TRAINING_ENGINE_DEPTH + 1), mode: 'solo_brilliant_white', requestId: Date.now() + 11 })
            : Promise.resolve(null);
        var blackTask = blackFen
            ? window.queueStockfishEval(blackFen, { depth: Math.max(14, SOLO_TRAINING_ENGINE_DEPTH + 1), mode: 'solo_brilliant_black', requestId: Date.now() + 12 })
            : Promise.resolve(null);
        var results = await Promise.all([whiteTask, blackTask]);
        if (token !== soloTrainingState.brilliantRequestToken || baseFen !== soloTrainingChess.fen()) return;
        soloTrainingState.brilliant.white = whiteFen ? detectSoloBrilliantMove('white', whiteFen, results[0]) : emptySoloBrilliant('Beyaz icin uygun degil');
        soloTrainingState.brilliant.black = blackFen ? detectSoloBrilliantMove('black', blackFen, results[1]) : emptySoloBrilliant('Siyah icin uygun degil');
        soloTrainingState.brilliant.stateText = (soloTrainingState.brilliant.white.found || soloTrainingState.brilliant.black.found) ? 'Bulundu' : 'Yok';
        syncSoloTrainingUI();
    } catch (e) {
...
async function refreshSoloTrainingStockfish(force) {
    ...
    if (!force && soloTrainingState.fen === baseFen && (soloTrainingState.best.white.uci || soloTrainingState.best.black.uci)) {
        syncSoloTrainingUI();
        scheduleSoloAutoMove();
        return;
    }
    ...
    if (isSoloGameOver()) {
        ...
        soloTrainingState.statusText = 'Konum tamamlandi; sifirlayarak yeni antrenmana baslayabilirsin.';
        syncSoloTrainingUI();
    ...
    soloTrainingState.statusText = 'Stockfish beyaz ve siyah icin en iyi devamlari ariyor...';
    soloTrainingState.evalText = '--';
    ...
    soloTrainingState.mistakes.white = emptySoloMistakeSet('Riskli hamleler taraniyor');
    soloTrainingState.mistakes.black = emptySoloMistakeSet('Riskli hamleler taraniyor');
    syncSoloTrainingUI();

    try {
        await window.initStockfish();
        var isWhiteTurn = soloTrainingChess.turn() === 'w';
        var primarySide = isWhiteTurn ? 'white' : 'black';
        var primaryFen = isWhiteTurn ? whiteFen : blackFen;
        var secondaryFen = isWhiteTurn ? blackFen : whiteFen;

        var primaryTask = primaryFen
            ? window.queueStockfishEval(primaryFen, { depth: SOLO_TRAINING_ENGINE_DEPTH, mode: 'solo_training_' + primarySide, priority: 'high', requestId: Date.now() + 1 })
            : Promise.resolve(null);
        ...
        if (isWhiteTurn) {
            soloTrainingState.best.white = whiteFen ? normalizeSoloBest(whiteFen, primaryResult) : emptySoloBest('Bu konum beyaz icin uygun degil');
            soloTrainingState.mistakes.white = whiteFen ? buildSoloMistakeCandidates(whiteFen, primaryResult) : emptySoloMistakeSet('Beyaz icin uygun degil');
            soloTrainingState.evalText = soloTrainingState.best.white.evalText || '--';
        } else {
            soloTrainingState.best.black = blackFen ? normalizeSoloBest(blackFen, primaryResult) : emptySoloBest('Bu konum siyah icin uygun degil');
            soloTrainingState.mistakes.black = blackFen ? buildSoloMistakeCandidates(blackFen, primaryResult) : emptySoloMistakeSet('Siyah icin uygun degil');
            soloTrainingState.evalText = soloTrainingState.best.black.evalText || '--';
        }
        soloTrainingState.mistakes.stateText = 'Canli';
        soloTrainingState.engineState = 'Hazir';
        soloTrainingState.statusText = 'Oneriye tiklayinca hamle tahtada kisa animasyonla isaretlenir.';
        syncSoloTrainingUI();
        scheduleSoloAutoMove();

        if (secondaryFen) {
            window.queueStockfishEval(secondaryFen, { depth: SOLO_TRAINING_ENGINE_DEPTH, mode: 'solo_training_' + (isWhiteTurn ? 'black' : 'white'), requestId: Date.now() + 2 }).then(function(secResult) {
                if (token !== soloTrainingState.requestToken) return;
                if (isWhiteTurn) {
                    soloTrainingState.best.black = normalizeSoloBest(secondaryFen, secResult);
                    soloTrainingState.mistakes.black = buildSoloMistakeCandidates(secondaryFen, secResult);
                } else {
                    soloTrainingState.best.white = normalizeSoloBest(secondaryFen, secResult);
                    soloTrainingState.mistakes.white = buildSoloMistakeCandidates(secondaryFen, secResult);
                }
                syncSoloTrainingUI();
            }).catch(function() {});
        }
        ...
        soloTrainingState.statusText = 'Motor sorgusu tamamlanamadi. Yenile butonunu deneyebilirsin.';
...
function clearSoloPreviewMarks() {
    ...
    if (!board) return;
    board.querySelectorAll('.solo-preview-from, .solo-preview-to').forEach(function(el) {
...
window.previewSoloTrainingMove = function(color) {
    ...
    if (!best || !best.uci) {
        if (window.showToast) window.showToast('Bu taraf icin gosterilecek hamle yok.', 'info');
        return;
    }
...
window.previewSoloMistakeMove = function(color, category) {
    ...
    var key = ['inaccuracy', 'mistake', 'blunder'].indexOf(category) !== -1 ? category : 'inaccuracy';
    var candidate = soloTrainingState.mistakes[side] && soloTrainingState.mistakes[side][key];
    if (!candidate || !candidate.found || !candidate.uci) {
        if (window.showToast) window.showToast('Bu seviyede gosterilecek riskli hamle yok.', 'info');
        return;
    }
...
window.previewSoloBrilliantMove = function(color) {
    ...
    if (!brilliant || !brilliant.found || !brilliant.uci) {
        if (window.showToast) window.showToast('Bu taraf icin brilliant aday yok.', 'info');
        return;
    }
...
window.setSoloTrainingAutoSide = function(side) {
    var normalized = ['none', 'white', 'black', 'both'].indexOf(side) !== -1 ? side : 'none';
...
window.toggleSoloMoveClassifications = function() {
    soloTrainingState.showClassifications = !soloTrainingState.showClassifications;
    drawSoloTrainingBoard();
    syncSoloTrainingUI();
};
...
function getSoloVoiceReadyPrompt() {
    return getSoloColorName(getSoloCurrentSide()) + ' sirasi. Ornek: e iki e dort, fil c dort, piyon e dort, geri al.';
}

function setSoloSyncState(nextState) {
    soloTrainingState.sync = Object.assign({}, soloTrainingState.sync, nextState || {});
}
...
function getSoloSyncInternalSourceSnapshot() {
    if (window.currentViewId === 'view-1v1-game' && current1v1Id && current1v1Data && current1v1Data.status === 'active') {
        var history1v1 = chess1v1.history();
        return {
            site: 'satrancsite',
            mode: '1v1',
            sourceLabel: 'Satrancsite 1v1 - ' + (current1v1Data.code || current1v1Id || 'Canli Mac'),
            title: document.title || 'Satrancsite 1v1',
            pageUrl: getSoloSyncPageUrl(),
            gameKey: ['satrancsite', '1v1', current1v1Data.code || current1v1Id || 'active'].join('|'),
            moves: history1v1,
            moveCount: history1v1.length,
            detectedAt: Date.now()
        };
    }
    if (window.currentViewId === 'view-2v2-game' && current2v2Id && current2v2Data && current2v2Data.status === 'active') {
        var history2v2 = chess.history();
        return {
            site: 'satrancsite',
            mode: '2v2',
            sourceLabel: 'Satrancsite 2v2 - ' + (current2v2Data.code || current2v2Id || 'Canli Mac'),
            title: document.title || 'Satrancsite 2v2',
            pageUrl: getSoloSyncPageUrl(),
            gameKey: ['satrancsite', '2v2', current2v2Data.code || current2v2Id || 'active'].join('|'),
            moves: history2v2,
...
function announceSoloSyncInternalSourceSnapshot(reason, requestId) {
    ...
    try {
        window.postMessage({
            ...
            payload: Object.assign({}, snapshot, { reason: reason || 'update', requestId: requestId || null })
        }, '*');
        return true;
    } catch (e) {
        return false;
...
function setSoloCloudSttStatus(text, configured) {
    ...
    if (typeof configured === 'boolean') soloTrainingState.cloudSttConfigured = configured;
...
function stopSoloVoiceMeter() {
    ...
    if (soloVoiceMeterStream) {
        soloVoiceMeterStream.getTracks().forEach(function(track) { track.stop(); });
    }
...
function stopSoloVoiceRecognitionStream() {
    if (soloVoiceRecognitionStream) {
        soloVoiceRecognitionStream.getTracks().forEach(function(track) { track.stop(); });
    }
...
async function prepareSoloVoiceRecognitionTrack() {
    ...
    try {
        soloVoiceRecognitionStream = await navigator.mediaDevices.getUserMedia(getSoloVoiceAudioConstraints());
        soloVoiceRecognitionTrack = soloVoiceRecognitionStream.getAudioTracks()[0] || null;
...
async function refreshSoloCloudSttHealth() {
    try {
        ...
        setSoloCloudSttStatus(soloTrainingState.cloudSttConfigured ? 'Hazir' : 'API anahtari yok', soloTrainingState.cloudSttConfigured);
...
async function refreshSoloVoiceDevices() {
    ...
    try {
        ...
        select.innerHTML = '<option value="">Varsayilan mikrofon</option>' + microphones.map(function(device, index) {
            var label = device.label || ('Mikrofon ' + (index + 1));
            return '<option value="' + window.escapeHtml(device.deviceId) + '">' + window.escapeHtml(label) + '</option>';
        }).join('');
        select.value = microphones.some(function(device) { return device.deviceId === current; }) ? current : '';
        soloTrainingState.voiceSettings.deviceId = select.value;
        setSoloVoiceHint(microphones.length
            ? 'Mikrofon listesi guncellendi. Sesli hamleyi ac ve kisaca soyle: e iki e dort.'
            : 'Mikrofon bulunamadi veya izin verilmedi.');
    } catch (e) {
        setSoloVoiceHint('Mikrofon listesi okunamadi. Tarayici izinlerini kontrol et.');
    }
}
...
window.setSoloVoiceDevice = function(deviceId) {
    ...
    setSoloVoiceHint(deviceId ? 'Secilen mikrofon testte ve bulut yedek STT tarafinda kullanilir.' : 'Varsayilan mikrofon secildi.');
    syncSoloTrainingUI();
};
...
window.testSoloVoiceMic = async function() {
    ...
    try {
        ...
        soloVoiceMeterStream = await navigator.mediaDevices.getUserMedia(constraints);
        ...
        setSoloVoiceHint('Mikrofon testi acik. Normal sesle e iki e dort de ve cubugu kontrol et.');
        function tick() {
        ...
        setSoloVoiceHint('Mikrofon testi baslatilamadi. Windows/Chrome mikrofon iznini kontrol et.');
    }
};
...
var SOLO_VOICE_RANK_ALIASES = {
    '1': '1', bir: '1', biri: '1', bire: '1', one: '1',
    '2': '2', iki: '2', ikisi: '2', ikiye: '2', two: '2',
    '3': '3', uc: '3', ucu: '3', uce: '3', three: '3',
    '4': '4', dort: '4', dordu: '4', dorde: '4', dord: '4', four: '4',
    '5': '5', bes: '5', bese: '5', besi: '5', five: '5',
    '6': '6', alti: '6', altiya: '6', altiyi: '6', six: '6',
    '7': '7', yedi: '7', yediye: '7', yediyi: '7', seven: '7',
    '8': '8', sekiz: '8', sekize: '8', sekizi: '8', eight: '8'
};
...
function normalizeSoloVoiceText(text) {
    ...
    return value
        ...
        .replace(/[^a-z0-9\s]/g, ' ')
        .replace(/\b(lutfen|please|hamle|oyna|oynar|oynasin|sur|sursun|move|play|from|to|square|kare|tas|piece)\b/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();
}
...
function stripSoloVoiceTokenSuffix(token) {
    ...
    return value.replace(/(lerden|lardan|den|dan|ten|tan|yla|yle|ye|ya|yi|yu|e|a)$/g, '');
}

function tokenizeSoloVoiceText(text) {
    var tokens = [];
    normalizeSoloVoiceText(text).split(/\s+/).filter(Boolean).forEach(function(rawToken) {
        ...
        var squareToken = clean.match(/^([a-h][1-8])(den|dan|ten|tan|ye|ya|e|a)?$/);
        if (squareToken) {
...
function extractSoloVoiceSquaresFromTokens(tokens) {
    ...
    return squares.filter(function(square, index) { return squares.indexOf(square) === index; });
}
...
function getSoloVoicePromotion(tokens) {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        if (SOLO_VOICE_PIECE_WORD_FORMS.q.indexOf(token) !== -1 || token === 'queen') return 'q';
        if (SOLO_VOICE_PIECE_WORD_FORMS.r.indexOf(token) !== -1 || token === 'rook') return 'r';
        if (SOLO_VOICE_PIECE_WORD_FORMS.b.indexOf(token) !== -1 || token === 'bishop') return 'b';
        if (SOLO_VOICE_PIECE_WORD_FORMS.n.indexOf(token) !== -1 || token === 'knight') return 'n';
    }
    return null;
}
...
function getSoloVoicePieceType(tokens) {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        for (var t = 0; t < types.length; t++) {
            if (SOLO_VOICE_PIECE_WORD_FORMS[types[t]].indexOf(token) !== -1) return types[t];
        }
    }
    return null;
}
...
function getSoloVoiceSquarePhrases(square) {
    ...
    return phrases.map(normalizeSoloVoiceText).filter(Boolean).filter(function(item, index, arr) {
...
function getSoloVoiceFilePhrases(file) {
    ...
    return variants.map(normalizeSoloVoiceText).filter(Boolean).filter(function(item, index, arr) {
        return arr.indexOf(item) === index;
    });
}
...
function getSoloVoicePieceWordForms(type) {
    ...
    return forms.concat(getSoloVoicePieceNames(type)).map(normalizeSoloVoiceText).filter(Boolean).filter(function(item, index, arr) {
        return arr.indexOf(item) === index;
    });
}
...
function normalizeSoloSanForVoice(san) {
    if (!san) return '';
    var value = String(san || '').replace(/[+#?!]/g, '').replace(/0/g, 'o');
    if (/^o-o-o$/i.test(value)) return 'uzun rok';
    if (/^o-o$/i.test(value)) return 'kisa rok';
    return normalizeSoloVoiceText(value.replace(/=/g, ' ').replace(/x/g, ' ').replace(/-/g, ' '));
}
...
function getSoloVoiceMovePhrases(move) {
    ...
    if (move.san) phrases.push(normalizeSoloSanForVoice(translateSanToTurkish(move.san)));
    return phrases.map(normalizeSoloVoiceText).filter(Boolean).filter(function(item, index, arr) {
        return arr.indexOf(item) === index;
    });
}

function scoreSoloVoiceMoveCandidate(move, normalizedText, tokens, squares, pieceType, promotion) {
    var score = 0;
    if (!move) return score;
    if (squares.length >= 2 && move.from === squares[0] && move.to === squares[1]) score = 190;
    else if (squares.length === 1 && move.to === squares[0]) score = Math.max(score, 90);
    ...
    for (var i = 0; i < phrases.length; i++) {
        ...
        if (normalizedText === phrase) {
            score = Math.max(score, phrase.indexOf(move.from) !== -1 || phrase.indexOf(spellSoloVoiceSquare(move.from)) !== -1 ? 180 : 150);
        } else if (normalizedText.indexOf(phrase) !== -1) {
            score = Math.max(score, 120);
        }
    }
    if (!pieceType && move.piece === 'p' && squares.length === 1 && move.to === squares[0]) score += 10;
    return score;
}
...
function pickSoloVoiceMoveFromTranscript(normalizedText) {
    ...
    var scored = legal.map(function(move) {
        return {
            move: move,
            score: scoreSoloVoiceMoveCandidate(move, normalizedText, tokens, squares, pieceType, promotion)
        };
    ...
    if (scored[1] && getSoloMoveUci(scored[0].move) !== getSoloMoveUci(scored[1].move)) {
        if (scored[0].score === scored[1].score) return { ambiguous: true };
        if (scored[0].score < 180 && scored[0].score - scored[1].score < 18) return { ambiguous: true };
    }
...
function findSoloCastleMove(text) {
    ...
    var wantShort = /\bkisa rok\b/.test(text) || /\bshort\b/.test(text) || !wantLong;
    var legal = soloTrainingChess.moves({ verbose: true }) || [];
    var castles = legal.filter(function(move) {
        var flags = move.flags || '';
        return (wantShort && flags.indexOf('k') !== -1) || (wantLong && flags.indexOf('q') !== -1);
    });
    if (!castles.length) return null;
    if (castles.length > 1 && !wantLong && !/\bkisa rok\b/.test(text) && !/\bshort\b/.test(text)) return { ambiguous: true };
    return castles[0];
}
...
function parseSoloVoiceMove(rawText) {
    ...
    if (!text) return null;
    if (/\b(geri al|geri git|geri don|bir onceki hamle|son hamleyi geri al|hamleyi geri al|undo|geri)\b/.test(text)) {
        return { action: 'undo', text: text };
    }
    if (/\b(ileri al|ileri git|yeniden oyna|tekrar oyna|hamleyi tekrar oyna|redo|ileri)\b/.test(text)) {
        return { action: 'redo', text: text };
    }
    if (/\b(en iyi kapat|motor kapat|panel kapat)\b/.test(text)) return { action: 'hideBest', text: text };
    if (/\b(en iyi ac|motor ac|panel ac)\b/.test(text)) return { action: 'showBest', text: text };

    var castle = findSoloCastleMove(text);
    if (castle && castle.ambiguous) return { ambiguous: true, text: text };
    if (castle) return { from: castle.from, to: castle.to, promotion: castle.promotion || null, san: castle.san, text: text };
...
function pickSoloVoiceParsedResult(result) {
    ...
    for (var i = 0; result && i < result.length; i++) {
        var transcript = result[i] && result[i].transcript ? result[i].transcript : '';
        var confidence = result[i] && typeof result[i].confidence === 'number' ? result[i].confidence : 0.75;
        var parsed = parseSoloVoiceMove(transcript);
        if (!parsed) continue;
        var picked = { parsed: parsed, transcript: transcript, confidence: confidence };
        if (!fallback) fallback = picked;
        if (parsed.action || parsed.ambiguous || confidence >= threshold) return picked;
    }
    return fallback;
}
...
function stopSoloCloudFallback() {
    ...
    if (soloVoiceCloudRecorder && soloVoiceCloudRecorder.state !== 'inactive') {
        try { soloVoiceCloudRecorder.stop(); } catch (e) {}
    }
    soloVoiceCloudRecorder = null;
    if (soloVoiceCloudStream) {
        soloVoiceCloudStream.getTracks().forEach(function(track) { track.stop(); });
    }
    soloVoiceCloudStream = null;
    setSoloCloudSttStatus(soloTrainingState.cloudSttConfigured ? 'Hazir' : soloTrainingState.cloudSttStatusText, soloTrainingState.cloudSttConfigured);
}

function scheduleSoloCloudCapture(delayMs) {
    if (soloVoiceCloudTimer) clearTimeout(soloVoiceCloudTimer);
    soloVoiceCloudTimer = setTimeout(runSoloCloudCapture, Math.max(120, delayMs || 250));
}

function isSoloVoiceDuplicateCommand(transcript) {
    var key = normalizeSoloVoiceText(transcript);
    var now = Date.now();
    var cooldownMs = Math.max(900, Number(soloTrainingState.voiceSettings.commandCooldownMs) || 2200);
    if (key && key === soloVoiceLastCommandKey && now - soloVoiceLastCommandAt < cooldownMs) return true;
...
async function transcribeSoloCloudBlob(blob) {
    ...
    try {
        ...
        if (!response.ok || !data.ok) {
            setSoloCloudSttStatus(response.status === 503 ? 'API anahtari yok' : 'STT hata', response.status !== 503);
            if (response.status === 503) setSoloVoiceHint('Bulut STT icin proxy OPENAI_API_KEY ile baslatilmali.');
            return;
        }
        ...
        setSoloVoiceHint('Bulut STT proxy baglantisi yok. Linki 8091 proxy uzerinden ac.');
    } finally {
        soloVoiceCloudBusy = false;
        syncSoloTrainingUI();
    }
...
async function runSoloCloudCapture() {
    ...
    if (!soloTrainingState.voiceEnabled || window.currentViewId !== 'view-solo-training') return;
    ...
    if (!soloVoiceCloudStream) {
        try {
            soloVoiceCloudStream = await navigator.mediaDevices.getUserMedia(getSoloVoiceAudioConstraints());
        } catch (e) {
    ...
    soloVoiceCloudRecorder.onstop = async function() {
        ...
        if (soloTrainingState.voiceEnabled && window.currentViewId === 'view-solo-training') {
            scheduleSoloCloudCapture(soloTrainingState.voiceSettings.cloudCooldownMs || 250);
        }
    };
    soloVoiceCloudRecorder.start();
    setSoloCloudSttStatus('Kayit aliyor', true);
    soloVoiceCloudTimer = setTimeout(function() {
        if (soloVoiceCloudRecorder && soloVoiceCloudRecorder.state !== 'inactive') {
            try { soloVoiceCloudRecorder.stop(); } catch (e) {}
        }
    }, Math.max(1200, Math.min(4500, soloTrainingState.voiceSettings.cloudRecordMs || 2300)));
}
...
async function startSoloCloudFallback() {
    ...
    setSoloVoiceHint('Tarayici duymazsa secili mikrofonla bulut yedek STT devreye girer.');
...
async function playSoloTrainingMoveFromInput(from, to, promotion, options) {
    ...
    if (isPromotionMoveForGame(soloTrainingChess, from, to) && !promotion && (!options || options.promptPromotion !== false)) {
...
function normalizeSoloImportedMoveToken(token) {
    ...
    value = value.replace(/[\u2654-\u265F]/g, function(ch) { return pieces[ch] || ''; });
    value = value
        .replace(/^[0-9]+\.(\.\.\.)?/, '')
        .replace(/[−–—]/g, '-')
...
function getSoloExternalSnapshotSignature(snapshot) {
    ...
    return String(snapshot.gameKey || 'game') + '|' + (Array.isArray(snapshot.moves) ? snapshot.moves.join(' ') : '');
}
...
function findSoloImportedLegalMove(game, token) {
    ...
    for (var i = 0; i < legal.length; i++) {
        ...
        var uci = String((move.from || '') + (move.to || '') + (move.promotion || '')).toLowerCase();
...
function canIncrementallyApplySoloSnapshot(snapshot) {
    var currentMoves = Array.isArray(soloTrainingState.sync.lastRawMoves) ? soloTrainingState.sync.lastRawMoves : [];
    var nextMoves = Array.isArray(snapshot && snapshot.moves) ? snapshot.moves : [];
    if (!soloTrainingState.sync.live) return false;
    if (!currentMoves.length && nextMoves.length) return false;
    if (String(soloTrainingState.sync.lastGameKey || '') !== String(snapshot && snapshot.gameKey || '')) return false;
    if (nextMoves.length < currentMoves.length) return false;
    for (var i = 0; i < currentMoves.length; i++) {
        if (String(currentMoves[i] || '') !== String(nextMoves[i] || '')) return false;
    }
    return nextMoves.length > currentMoves.length;
}

function waitForSoloTrainingView(timeoutMs) {
    if (window.currentViewId === 'view-solo-training') return Promise.resolve(true);
...
function applySoloImportedMoveToGame(game, rawToken) {
    ...
    if (!token || token === '...' || isSoloImportedResultToken(token)) return null;
    ...
    if (/^[kqrbn]?[a-h]?[1-8]?[-x]?[a-h][1-8](=?[nbrq])?[+#]?$/i.test(token) || /^[a-h][1-8][-x][a-h][1-8][nbrq]?$/i.test(token)) {
        ...
        if (/^[a-h][1-8][a-h][1-8][nbrq]?$/i.test(compact)) {
            return game.move({
                ...
                promotion: compact.length > 4 ? compact.slice(4, 5).toLowerCase() : undefined
            });
        }
    }
...
async function applySoloExternalSnapshot(snapshot) {
    ...
    if (signature === soloTrainingState.sync.lastSignature && !snapshot.force) return true;
    ...
    try {
        if (window.currentViewId !== 'view-solo-training' && typeof window.openSoloTrainingMode === 'function') {
        ...
        if (!snapshot.force && canIncrementallyApplySoloSnapshot(snapshot)) {
            incremental = true;
            var currentLen = Array.isArray(soloTrainingState.sync.lastRawMoves) ? soloTrainingState.sync.lastRawMoves.length : 0;
        ...
        setSoloSyncState({ applying: false });
        if (_soloSyncApplyLockTimer) { clearTimeout(_soloSyncApplyLockTimer); _soloSyncApplyLockTimer = null; }
        syncSoloTrainingUI();
        ...
        setSoloSyncState({
            ...
            live: wasLiveSync,
            stateText: 'Hata',
            statusText: 'Canli aktarim hatasi: ' + (error && error.message ? error.message : 'Bilinmeyen hata'),
            sourceLabel: snapshot.sourceLabel || snapshot.site || 'Harici tahta'
        });
        if (_soloSyncApplyLockTimer) { clearTimeout(_soloSyncApplyLockTimer); _soloSyncApplyLockTimer = null; }
        syncSoloTrainingUI();
...
window.addEventListener('message', function(event) {
    ...
    if (data.type === 'SOLO_SYNC_EXTENSION_REQUEST_SOURCE_SNAPSHOT') {
        announceSoloSyncInternalSourceSnapshot('extension-request', data.requestId || null);
        return;
    }
...
async function handleSoloVoiceTranscript(transcript) {
    ...
    if (parsed.ambiguous) {
        setSoloVoiceStatus('Belirsiz hamle. Kaynak kareyi de soyle: g bir f uc gibi.');
        return;
    }
    ...
    try {
        var ok = await playSoloTrainingMoveFromInput(parsed.from, parsed.to, parsed.promotion, { promptPromotion: false });
        setSoloVoiceStatus(ok
            ? movingSide + ' oynadi: ' + formatSoloMoveText(parsed.san || (parsed.from + '-' + parsed.to))
...
function stopSoloVoiceControl() {
    ...
    if (soloVoiceRecognition) {
        try { soloVoiceRecognition.onend = null; soloVoiceRecognition.stop(); } catch (e) {}
    }
...
function armSoloVoiceWatchdog() {
    ...
    soloVoiceWatchdogTimer = setTimeout(function() {
        ...
        if (soloVoiceRecognition) {
            try { soloVoiceRecognition.abort(); return; } catch (e) {}
        }
        startSoloVoiceControl();
    }, 11000);
}
...
async function startSoloVoiceControl() {
    ...
    if (!recognitionTrack) {
        setSoloVoiceHint('Sesli hamle icin mikrofon izni gerekli. Tarayici isterse izin ver.');
    }
    await refreshSoloVoiceDevices();
    ...
    if (!SpeechCtor && !cloudConfigured) {
        ...
        setSoloVoiceStatus('Tarayici ses algilamayi baslatamadi. Chrome veya Edge ile dene.');
        syncSoloTrainingUI();
        return;
    }
    if (!SpeechCtor) {
    ...
    soloVoiceRecognition.maxAlternatives = Math.max(1, Math.min(5, soloTrainingState.voiceSettings.maxAlternatives || 4));
    ...
    soloVoiceRecognition.onspeechstart = function() {
        ...
        setSoloVoiceStatus('Ses algilandi. ' + getSoloColorName(getSoloCurrentSide()) + ' hamlesi bekleniyor...');
        armSoloVoiceWatchdog();
    };
    ...
    soloVoiceRecognition.onerror = function(event) {
        ...
        if (error === 'aborted' || error === 'no-speech') {
            setSoloVoiceStatus(error === 'aborted' ? 'Dinleme yenileniyor...' : 'Ses algilanmadi. ' + getSoloVoiceReadyPrompt());
            return;
        }
        if (error === 'not-allowed' || error === 'service-not-allowed') {
            setSoloVoiceStatus(cloudConfigured ? 'Tarayici izni yok, bulut yedek STT calisiyor...' : 'Tarayici mikrofona izin vermedi.');
    ...
    soloVoiceRecognition.onend = function() {
        ...
        soloTrainingState.voiceListening = false;
        if (soloTrainingState.voiceEnabled && window.currentViewId === 'view-solo-training') {
            var delay = Math.max(100, Math.min(2000, soloTrainingState.voiceSettings.restartDelayMs || 450));
    ...
    soloVoiceRecognition.onresult = function(event) {
        ...
        var alternatives = [];
        var liveText = '';
        for (var i = event.resultIndex || 0; event.results && i < event.results.length; i++) {
            ...
            for (var j = 0; j < result.length; j++) {
                alternatives.push({
                    transcript: result[j] && result[j].transcript ? result[j].transcript : '',
                    confidence: result[j] && typeof result[j].confidence === 'number' ? result[j].confidence : 0.75
                });
            }
        }
        ...
        if (!picked) {
            var heard = alternatives[0] && alternatives[0].transcript ? alternatives[0].transcript : '';
            setSoloVoiceLastHeard(heard);
            setSoloVoiceStatus('Komut eslesmedi. ' + getSoloVoiceReadyPrompt());
            return;
        }
        var confidenceText = typeof picked.confidence === 'number' ? ' (' + picked.confidence.toFixed(2) + ')' : '';
        setSoloVoiceStatus('Duyuldu: ' + picked.transcript + confidenceText);
        setSoloVoiceLastHeard(picked.transcript);
        if (!isSoloVoiceDuplicateCommand(picked.transcript)) handleSoloVoiceTranscript(picked.transcript, 'native');
    };
    try {
        if (recognitionTrack) {
            soloVoiceRecognition.start(recognitionTrack);
            setSoloVoiceHint(soloTrainingState.voiceSettings.deviceId
            ...
            setSoloVoiceHint('Tarayici kendi varsayilan mikrofonu ile dinliyor. Gerekirse Windows varsayilan mikrofonunu degistir.');
        }
    } catch (e) {
        if (recognitionTrack) {
            try {
                soloVoiceRecognition.start();
                stopSoloVoiceRecognitionStream();
                setSoloVoiceHint('Bu tarayici secili mikrofonu dogrudan desteklemedi; sistem varsayilan mikrofonuyla dinleniyor.');
            } catch (fallbackError) {
                if (cloudConfigured) {
                    stopSoloVoiceRecognitionStream();
                    setSoloVoiceEngineText('Bulut');
...
function chooseSoloPromotion(color) {
    return new Promise(function(resolve) {
        ...
        ['q', 'r', 'b', 'n'].forEach(function(type) {
            ...
            else piece.style.backgroundImage = "url('https://images.chesscomfiles.com/chess-themes/pieces/neo/150/" + color + type + ".png')";
...
async function handleSoloTrainingSquareClick(squareName) {
    ...
    if (soloTrainingValidMoves.indexOf(squareName) !== -1 && soloTrainingSelectedSquare) {
        var played = await playSoloTrainingMoveFromInput(soloTrainingSelectedSquare, squareName, null, { promptPromotion: true });
        if (played) {
            return;
        }
    }
    ...
    if (piece && piece.color === soloTrainingChess.turn()) {
        ...
        soloTrainingValidMoves = soloTrainingChess.moves({ square: squareName, verbose: true }).map(function(move) { return move.to; });
    } else {
        soloTrainingSelectedSquare = null;
        soloTrainingValidMoves = [];
    }
...
window.redoSoloTrainingMove = function() {
    ...
    if (nextMove.classification) {
        ...
        recordSoloMoveClassification(played, played.color === 'w' ? 'white' : 'black', soloTrainingChess.fen());
    }
    soloTrainingSelectedSquare = null;
    soloTrainingValidMoves = [];
    resetSoloEngineState('Bekleniyor');
    drawSoloTrainingBoard();
    if (window.playChessMoveSound) window.playChessMoveSound(played, soloTrainingChess);
    else if (window.playGameSound) window.playGameSound('move');
    syncSoloTrainingUI();
    refreshSoloTrainingStockfish(true);
};
...
window.openSoloTrainingMode = function() {
    syncSoloTrainingAccess();
    if (!isSoloTrainingAllowed()) {
        if (window.showToast) window.showToast('Bu ozel mod yalnizca yetkili hesapta acilir.', 'error');
        return;
    }
...
function update1v1Game(data) {
    ...
    if (data.status === 'finished') warmAnalysisCacheForActiveGame('1v1', data.pgn || '', chess1v1.fen(), data.moveCount || 0);
    ...
    if (data.moveCount > lastPlayedMoveCount1v1) {
        if (lastPlayedMoveCount1v1 !== -1) {
            ...
            if (window.playChessMoveSoundFromFlags) window.playChessMoveSoundFromFlags(flags, chess1v1, data.isCheck === true);
            else window.playGameSound('move');
        }
    ...
    lastRenderedLiveMoveCount1v1 = renderLiveMovePanel({
        ...
        statusText: data.status === 'finished'
            ? 'Mac tamamlandi'
            : (current1v1Role === 'spectator' ? 'Izleyici modu' : 'Canli oyun'),
...
function render1v1BoardInto(boardEl) {
    ...
    if (needsInit) {
        boardEl.innerHTML = '';
        boardEl.dataset.rotate = String(rotate);
    }
    ...
    for (let r = 0; r < 8; r++) {
        for (let c = 0; c < 8; c++) {
            ...
            if (needsInit) {
                ...
                div = boardEl.children[squareIdx];
            }
            squareIdx++;
            ...
            let piece = div.querySelector('.piece');
            if (sq) {
                const pieceKey = sq.color + sq.type + '_' + (isMyTurn ? '1' : '0');
                if (!piece) {
                    ...
                    div.appendChild(piece);
                } else if (piece.dataset.pieceKey !== pieceKey) {
                    piece.className = 'piece ' + (isMyTurn ? 'active' : 'locked');
                    piece.dataset.pieceKey = pieceKey;
                    if (window.applyPieceSkin) window.applyPieceSkin(piece, sq.color, sq.type);
                    else piece.style.backgroundImage = `url('https://images.chesscomfiles.com/chess-themes/pieces/neo/150/${sq.color}${sq.type}.png')`;
...
function update2v2Game(d) {
    ...
    if(d.moveCount > lastPlayedMoveCount) {
        if(lastPlayedMoveCount !== -1) {
            ...
            if (window.playChessMoveSoundFromFlags) window.playChessMoveSoundFromFlags(flags, chess, d.isCheck === true);
            else window.playGameSound('move');
        }
...
function renderReconnectBanner(mode, data) {
    ...
    ids.forEach(function(id) {
        ...
        var message = disconnected.map(function(item) {
            ...
            return name + ' bağlantıyı kaybetti. ' + left + ' sn içinde dönmezse maç sonuçlanacak.';
        }).join(' ');
...
window.copySpectatorLink = function(mode) {
    ...
    var base = window.getAppBaseUrl ? window.getAppBaseUrl() : (window.location.origin + window.location.pathname);
    var url = base + (base.indexOf('?') >= 0 ? '&' : '?') + 'watchMode=' + encodeURIComponent(mode) + '&watchCode=' + encodeURIComponent(code);
...
function buildChessArchiveCandidates(headers) {
    var seen = {};
    var candidates = [];
    [headers && headers.UTCDate, headers && headers.Date].forEach(function(item) {
...
async function fetchChessComCallbackGame(gameId) {
    var targetUrl = 'https://www.chess.com/callback/live/game/' + encodeURIComponent(gameId);
    var proxyUrls = [
...
async function fetchChessComArchive(username, year, month) {
    var url = 'https://api.chess.com/pub/player/' + encodeURIComponent(String(username || '').toLowerCase()) + '/games/' + year + '/' + month;
...
function findChessComArchiveGame(archivePayload, gameId) {
    var games = Array.isArray(archivePayload && archivePayload.games) ? archivePayload.games : [];
    ...
    for (var i = 0; i < games.length; i++) {
        ...
        if (String(game.pgn || '').indexOf('/game/live/' + wanted) !== -1) return game;
    }
    return null;
}

async function resolveChessComArchiveGame(metaPayload, gameId) {
    var headers = metaPayload && metaPayload.game ? (metaPayload.game.pgnHeaders || {}) : {};
    var players = [];
    if (headers.White) players.push(headers.White);
    if (headers.Black && headers.Black !== headers.White) players.push(headers.Black);

    var archiveDates = buildChessArchiveCandidates(headers);
    if (!players.length || !archiveDates.length) throw new Error('archive_lookup_unavailable');

    for (var i = 0; i < players.length; i++) {
        for (var j = 0; j < archiveDates.length; j++) {
            try {
                var archive = await fetchChessComArchive(players[i], archiveDates[j].year, archiveDates[j].month);
...
function buildImportedPlayers(game, metaPayload) {
    ...
    return [
        {
            team: 'white',
            name: (game && game.white && game.white.username) || headers.White || 'White',
            avatar: 'fa-chess-king'
        },
        {
            team: 'black',
            name: (game && game.black && game.black.username) || headers.Black || 'Black',
...
async function importChessComGameByUrl(inputValue) {
    ...
    window.openAnalysis(archiveGame.pgn, importedPlayers, archiveGame.initial_setup || archiveGame.fen || null);
...
window.openChessComImportPrompt = async function() {
    var result = await Swal.fire({
        ...
        inputPlaceholder: 'https://www.chess.com/analysis/game/live/169528019640/analysis?flip=true',
        background: 'rgba(30,30,35,0.95)',
        color: '#fff',
        ...
        inputValidator: function(value) {
            return parseChessComGameId(value) ? undefined : 'Gecerli bir Chess.com canli mac linki gir.';
        }
    });
    ...
    try {
        ...
        Swal.close();
        window.showToast(importedPlayers[0].name + ' vs ' + importedPlayers[1].name + ' maci ice aktarildi.', 'success');
    } catch (error) {
        console.error(error);
        Swal.close();
        ...
        if (error && error.message === 'archive_game_not_found') {
            window.showToast('Mac bulundu ama Chess.com arsivinde PGN alinmadi.', 'error');
            return;
        }
        window.showToast('Chess.com maci ice aktarilamadi. Linki kontrol edip tekrar dene.', 'error');
    }
};
    ...
    try {
        ...
        if (remainingMs <= 0) {
            if (await forfeitSavedActiveGame(saved, data, 'disconnect_timeout')) {
                window.showToast('Yeniden bağlanma süresi doldu. Maç hükmen sonuçlandırıldı.', 'warning');
            }
            return false;
        }
        ...
        var result = await Swal.fire({
            ...
            html: '<div style="text-align:left; line-height:1.6;">'
                + '<div><strong>' + window.escapeHtml(window.getModeLabel(saved.mode)) + '</strong> maçın hâlâ aktif görünüyor.</div>'
...
function formatQuizMoney(value) {
    ...
    return amount.toLocaleString('tr-TR', { style: 'currency', currency: 'TRY', maximumFractionDigits: amount % 1 ? 2 : 0 });
}
...
function normalizeQuizTest(raw) {
    ...
    const questions = Array.isArray(data.questions)
        ? data.questions.map(normalizeQuizQuestion).filter(q => q.q && !q.opts.some(o => !o))
        : [];
    const totalValue = questions.reduce((sum, q) => sum + normalizeQuizMoneyValue(q.value), 0);
    return {
        id: data.id || '',
        name: String(data.name || data.title || 'Kayitli Test').trim() || 'Kayitli Test',
        questions: questions,
        questionCount: questions.length,
        totalValue: normalizeQuizMoneyValue(data.totalValue != null ? data.totalValue : totalValue),
        savedAtMs: Math.max(0, parseInt(data.savedAtMs || data.updatedAtMs || 0, 10) || 0)
    };
}
...
function normalizeQuizTestLibraryItem(raw, fallbackId) {
    ...
    test.id = (raw && raw.id) || test.id || fallbackId || getQuizTestFingerprint(test);
    return {
        id: test.id,
        test: test
    };
...
function saveLocalQuizTestLibraryItem(uid, test) {
    try {
        const item = normalizeQuizTestLibraryItem({ id: test.id, test: test }, test.id);
        if (!item) return false;
        const existing = readLocalQuizTestLibrary(uid).map((entry, index) => normalizeQuizTestLibraryItem(entry, 'local-' + index)).filter(Boolean);
        const next = [item].concat(existing).filter((entry, index, list) => {
            const key = getQuizTestFingerprint(entry.test);
            return list.findIndex(other => getQuizTestFingerprint(other.test) === key) === index;
        }).slice(0, 60);
...
window.updateQData = (idx, field, val, subIdx = null) => {
    ...
    if(field === 'value') quizBuilderQuestions[idx].value = normalizeQuizMoneyValue(val);
};
...
window.saveQuizQuestionToLibrary = async (idx) => {
    ...
    if (!q.q || q.opts.some(o => !o)) {
        window.showToast("Kütüphaneye eklemek için soru ve seçenekleri doldur.", "error");
        return;
    }
    ...
    try {
        ...
        console.warn("Quiz library subcollection write failed, trying profile fallback.", e);
        try {
...
async function loadQuizQuestionLibrary() {
    ...
    try {
        const snap = await getDocs(collection(db, "users", uid, "quiz_question_library"));
        snap.docs.forEach(docSnap => {
            const item = normalizeQuizLibraryItem(docSnap.data() || {}, docSnap.id);
            if (item) items.push(item);
        });
    } catch (e) {
        console.warn("Quiz library subcollection read failed, using profile fallback.", e);
    }

    try {
        const profileSnap = await getDoc(doc(db, "profiles", uid));
        const profileData = profileSnap.exists() ? (profileSnap.data() || {}) : {};
        const profileLibrary = Array.isArray(profileData.quizQuestionLibrary) ? profileData.quizQuestionLibrary : [];
        profileLibrary.forEach((entry, index) => {
            const item = normalizeQuizLibraryItem(entry, entry && entry.id ? entry.id : ('profile-' + index));
            if (item) items.push(item);
        });
    } catch (e) {
...
window.openQuizQuestionLibrary = async () => {
    ...
    list.innerHTML = '<div class="quiz-library-empty">Kütüphane yükleniyor...</div>';
    try {
        const items = await loadQuizQuestionLibrary();
        if (!items.length) {
            list.innerHTML = '<div class="quiz-library-empty">Henüz kayıtlı soru yok. Sorunun yanındaki yer imi butonuyla ekleyebilirsin.</div>';
            return;
        }
        list.innerHTML = items.map((item, i) => {
            const q = item.question;
            return '<div class="quiz-library-item">'
                + (q.img ? '<img src="' + q.img + '" alt="">' : '<div class="quiz-library-icon"><i class="fas fa-question"></i></div>')
                + '<div class="quiz-library-copy">'
                    + '<strong>' + window.escapeHtml(q.q) + '</strong>'
                    + '<span>' + q.opts.map((o, idx) => window.escapeHtml((idx === q.correct ? '✓ ' : '') + o)).join(' • ') + '</span>'
                    + '<small>' + q.time + ' sn • ' + formatQuizMoney(q.value) + '</small>'
                + '</div>'
                + '<button class="secondary quiz-library-use-btn" onclick="useQuizLibraryQuestion(' + i + ')"><i class="fas fa-plus"></i></button>'
                + '</div>';
        }).join('');
    } catch (e) {
        console.error(e);
        list.innerHTML = '<div class="quiz-library-empty">Kütüphane okunamadı.</div>';
    }
};
...
async function saveQuizTestToLibrary(name, questions, options = {}) {
    ...
    try {
        await addDoc(collection(db, "users", uid, "quiz_test_library"), {
            test: payload,
            createdAt: serverTimestamp(),
            updatedAtMs: Date.now()
        });
        quizTestLibraryCache = [];
        if (!options.silent) window.showToast("Test kütüphaneye kaydedildi.", "success");
        return true;
    } catch (e) {
        console.warn("Quiz test library subcollection write failed, trying profile fallback.", e);
        try {
            await setDoc(doc(db, "profiles", uid), {
            ...
            if (!options.silent) {
                window.showToast(localSaved ? "Test bu cihaza kaydedildi." : "Test kütüphaneye kaydedilemedi.", localSaved ? "success" : "error");
            }
            return localSaved;
        }
    }
...
async function loadQuizTestLibrary() {
    ...
    try {
        const snap = await getDocs(collection(db, "users", uid, "quiz_test_library"));
        snap.docs.forEach(docSnap => {
            const item = normalizeQuizTestLibraryItem(docSnap.data() || {}, docSnap.id);
            if (item) items.push(item);
        });
    } catch (e) {
        console.warn("Quiz test library subcollection read failed, using profile fallback.", e);
    }

    try {
        const profileSnap = await getDoc(doc(db, "profiles", uid));
        const profileData = profileSnap.exists() ? (profileSnap.data() || {}) : {};
        const profileLibrary = Array.isArray(profileData.quizTestLibrary) ? profileData.quizTestLibrary : [];
        profileLibrary.forEach((entry, index) => {
            const item = normalizeQuizTestLibraryItem(entry, entry && entry.id ? entry.id : ('profile-test-' + index));
            if (item) items.push(item);
        });
    } catch (e) {
    ...
    readLocalQuizTestLibrary(uid).forEach((entry, index) => {
        const item = normalizeQuizTestLibraryItem(entry, entry && entry.id ? entry.id : ('local-test-' + index));
        if (item) items.push(item);
    });

    const seen = new Set();
    quizTestLibraryCache = items.filter(item => {
...
window.openQuizTestLibrary = async () => {
    ...
    if (!panel || !list) return;
    panel.style.display = 'block';
    list.innerHTML = '<div class="quiz-library-empty">Test kütüphanesi yükleniyor...</div>';
    try {
        const items = await loadQuizTestLibrary();
        if (!items.length) {
            list.innerHTML = '<div class="quiz-library-empty">Henüz kayıtlı test yok. Bir quiz hazırlayıp lobi açtığında otomatik buraya kaydedilecek.</div>';
            return;
        }
        list.innerHTML = items.map((item, i) => {
            const test = item.test;
            const preview = test.questions.slice(0, 2).map(q => q.q).join(' • ');
            return '<div class="quiz-library-item quiz-test-library-item">'
                + '<div class="quiz-library-icon"><i class="fas fa-layer-group"></i></div>'
                + '<div class="quiz-library-copy">'
                    + '<strong>' + window.escapeHtml(test.name) + '</strong>'
                    + '<span>' + window.escapeHtml(preview || 'Sorular hazır') + '</span>'
                    + '<small>' + test.questionCount + ' soru • Toplam ' + formatQuizMoney(test.totalValue) + '</small>'
                + '</div>'
                + '<button class="secondary quiz-library-use-btn" onclick="startQuizFromTestLibrary(' + i + ')" title="Lobi aç"><i class="fas fa-play"></i></button>'
                + '</div>';
        }).join('');
    } catch (e) {
        console.error(e);
        list.innerHTML = '<div class="quiz-library-empty">Test kütüphanesi okunamadı.</div>';
    }
};
...
function buildQuizLobbyData(name, questions, hostPlays) {
    ...
    if (hostPlays) {
         initialPlayers.push({ uid: window.currentUser.uid, name: window.currentUser.displayName, avatar: window.currentUser.photoURL, score: 0, moneyWon: 0, answers: {} });
    }
    return {
        code: code,
        name: name,
        ...
        settings: { speedBonus: true, hostParticipate: hostPlays, autoNextQuestion: true },
        createdAt: serverTimestamp(),
        startTime: null
    };
}
...
window.saveAndStartQuiz = async () => {
    ...
    try {
        await setDoc(doc(db, "games_quiz", quizData.code), quizData);
        saveQuizTestToLibrary(name, quizBuilderQuestions, { silent: true }).then(saved => {
            if (saved) window.showToast("Test kütüphaneye otomatik kaydedildi.", "success");
        });
        window.enterQuizGame(quizData.code);
    } catch(e) {
        console.error(e);
...
window.enterQuizGame = (code) => {
    ...
    unsubscribeQuiz = onSnapshot(doc(db, "games_quiz", code), async (snap) => {
        ...
        if(d.hostId !== window.currentUser.uid && !myPlayer && d.status === 'lobby') {
            const newPlayer = { uid: window.currentUser.uid, name: window.currentUser.displayName, avatar: window.currentUser.photoURL, score: 0, moneyWon: 0, answers: {} };
...
function renderQuizLobby(d) {
    ...
    if(isHost) {
        ...
        document.getElementById('hostParticipateToggle').checked = settings.hostParticipate !== false;
        const autoNextToggle = document.getElementById('quizAutoNextToggle');
        if (autoNextToggle) autoNextToggle.checked = settings.autoNextQuestion !== false;
    }
...
window.toggleHostParticipation = async (shouldParticipate) => {
    ...
    if(shouldParticipate) {
         if(!newPlayers.find(p => p.uid === window.currentUser.uid)) {
             newPlayers.push({ uid: window.currentUser.uid, name: window.currentUser.displayName, avatar: window.currentUser.photoURL, score: 0, moneyWon: 0, answers: {} });
         }
    } else {
...
function renderQuizGame(d) {
    ...
    document.getElementById('quizScoreDisplay').innerText = (myP ? myP.score : 0) + " Puan • " + formatQuizMoney(myP ? (myP.moneyWon || 0) : 0);
    ...
    if (valueBadge) valueBadge.innerText = 'Soru Değeri: ' + formatQuizMoney(qData.value || 0);
    ...
    if(d.state === 'question') {
        ...
        if (quizForceNextProcessingKey && quizForceNextProcessingKey !== timerKey) quizForceNextProcessingKey = null;
        ...
        if (el && lastQuizTimerRenderKey !== timerKey) {
            ...
            el.style.width = Math.max(0, Math.min(100, (remainingMs / questionMs) * 100)) + '%';
        ...
        const autoNextQuestion = !d.settings || d.settings.autoNextQuestion !== false;
        const autoNextTimer = document.getElementById('autoNextTimer');
        if (autoNextTimer) {
            autoNextTimer.style.display = 'block';
            autoNextTimer.innerText = autoNextQuestion ? 'Sonraki soruya geçiliyor...' : 'Yönetici sıradaki soruya geçecek...';
        }
        const manualNextBtn = document.getElementById('quizManualNextQuestionBtn');
        if (manualNextBtn) manualNextBtn.style.display = (isHost && !autoNextQuestion) ? 'inline-flex' : 'none';
...
window.quizForceNext = async () => {
    ...
    if(currentQuizData.state === 'question') {
        ...
        const useBonus = !currentQuizData.settings || currentQuizData.settings.speedBonus !== false;
...
function renderQuizResults(d) {
    ...
    if(p1) { document.getElementById('name-1').innerText = p1.name; document.getElementById('score-1').innerText = (p1.score || 0) + ' P • ' + formatQuizMoney(p1.moneyWon || 0); document.getElementById('av-1').innerHTML = `<i class="fas ${p1.avatar || 'fa-user'}"></i>`; }
    if(p2) { document.getElementById('name-2').innerText = p2.name; document.getElementById('score-2').innerText = (p2.score || 0) + ' P • ' + formatQuizMoney(p2.moneyWon || 0); document.getElementById('av-2').innerHTML = `<i class="fas ${p2.avatar || 'fa-user'}"></i>`; } else { const el = document.querySelector('.podium-2'); if (el) el.style.opacity = '0'; }
    if(p3) { document.getElementById('name-3').innerText = p3.name; document.getElementById('score-3').innerText = (p3.score || 0) + ' P • ' + formatQuizMoney(p3.moneyWon || 0); document.getElementById('av-3').innerHTML = `<i class="fas ${p3.avatar || 'fa-user'}"></i>`; } else { const el = document.querySelector('.podium-3'); if (el) el.style.opacity = '0'; }
