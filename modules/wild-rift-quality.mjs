...
export function guideQuality(data,champion,role,now=Date.now(),variant=''){
  const build=champion?.builds?.find(b=>b.role===role&&b.guideId===variant)||champion?.builds?.find(b=>b.role===role),issues=[];
  ...
  if(champion?.refreshFailed)issues.push('Son kontrolde bu rehber yenilenemedi.');
  if(ageInDays(champion?.fetchedAt,now)>7)issues.push('Rehber yedi gündür doğrulanmadı.');
  ...
  const usable=!!build&&build.patch===data.latestPatch?.version&&!champion.refreshFailed&&ageInDays(champion.fetchedAt,now)<=7;
