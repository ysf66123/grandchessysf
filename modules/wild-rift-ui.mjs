import {sourcesView,priceProof} from './wild-rift-source-ui.mjs?v=20260923-wr3';
import {staticMode,publishedSnapshot} from './wild-rift-static.mjs?v=20260923-wr3';
import {itemAvailability,priceEvidence} from './wild-rift-evidence.mjs?v=20260923-wr3';
import {emptyDraft,sanitizeDraft,recommendations,recommendBuild,coaching,matchupPlan,threats,freshness,laneScenarios,movePick,SORT_MODES,ROLES} from './wild-rift-engine.mjs?v=20260923-wr3';
import {dataQuality} from './wild-rift-quality.mjs?v=20260923-wr3';
import {createHistory,readWorkspace,writeWorkspace} from './wild-rift-workspace.mjs?v=20260923-wr3';
import {purchasePlan,itemCost} from './wild-rift-purchase.mjs?v=20260923-wr3';
import {RANKS,traits,BOOT_UPGRADES} from './wild-rift-knowledge.mjs?v=20260923-wr3';
import {itemName,termName} from './wild-rift-tr.mjs?v=20260923-wr3';
...
export async function mount(){
  ...
  if(!document.getElementById('wr-css')){const link=document.createElement('link');link.id='wr-css';link.rel='stylesheet';link.href=new URL('../wild-rift.css?v=20260923-wr3',import.meta.url).href;document.head.append(link);}
...
function buildView(){
  ...
  if(result.missing&&result.invalidItems)return `<div class="wr-empty">Kaynak diziliminde kaldırılan eşya var: ${result.invalidItems.map(id=>esc(itemName(id))).join(', ')}. Bu dizilim önerilmiyor.</div>`;
...
async function refresh(){
  ...
  try{
    if(staticMode()){
      ...
      accept(next);render();message(before===data.checkedAt?'Yayımlanan son paketi kullanıyorsun. Kaynak taraması GitHub Actions üzerinden altı saatte bir yapılır.':'Yeni yayımlanan veri paketi yüklendi.');return;
    }
