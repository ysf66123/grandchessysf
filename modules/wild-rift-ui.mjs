import {emptyDraft,sanitizeDraft,recommendations,recommendBuild,coaching,matchupPlan,threats,freshness,laneScenarios,movePick,SORT_MODES,ROLES} from './wild-rift-engine.mjs?v=20260923-wr2';
import {dataQuality} from './wild-rift-quality.mjs?v=20260923-wr2';
import {createHistory,readWorkspace,writeWorkspace} from './wild-rift-workspace.mjs?v=20260923-wr2';
import {purchasePlan,itemCost} from './wild-rift-purchase.mjs?v=20260923-wr2';
import {RANKS,traits,BOOT_UPGRADES} from './wild-rift-knowledge.mjs?v=20260923-wr2';
import {itemName,termName} from './wild-rift-tr.mjs?v=20260923-wr2';
...
let workspace={saved:[],feedback:[]},showAll=false,renderTimes=[],feedbackId='',serviceStatus=null;
...
export async function mount(){
  ...
  if(!document.getElementById('wr-css')){const link=document.createElement('link');link.id='wr-css';link.rel='stylesheet';link.href=new URL('../wild-rift.css?v=20260923-wr2',import.meta.url).href;document.head.append(link);}
  ...
  if(!serviceStatus)api('/status').then(status=>{if(active())serviceStatus=status;}).catch(()=>{});
}
function header(){
...
async function refresh(){
  ...
  try{
    ...
    while(status.running&&active()&&attempt++<180){
