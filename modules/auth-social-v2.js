...
Object.defineProperties(window, {
    currentUser: { get: () => currentUser, set: (val) => { currentUser = val; } },
    currentProfileData: { get: () => currentProfileData, set: (val) => { currentProfileData = val; } },
    friendProfilesCache: { get: () => friendProfilesCache, set: (val) => { friendProfilesCache = val; } },
    friendRequestsCache: { get: () => friendRequestsCache, set: (val) => { friendRequestsCache = val; } },
    friendInvitesCache: { get: () => friendInvitesCache, set: (val) => { friendInvitesCache = val; } },
    notificationsCache: { get: () => notificationsCache, set: (val) => { notificationsCache = val; } },
    activeChatThreadId: { get: () => activeChatThreadId, set: (val) => { activeChatThreadId = val; } },
    unsubscribeChat: { get: () => unsubscribeChat, set: (val) => { unsubscribeChat = val; } }
});
...
function normalizeStoryCompletedMap(value) {
    ...
    Object.keys(value).forEach(function(missionId) {
        ...
        normalized[missionId] = {
            ...
            lastCompletedAtMs: Math.max(0, parseInt(entry.lastCompletedAtMs || 0, 10) || 0)
        };
    });
    return normalized;
}
...
function mergeStoryCompletedMap(primary, fallback) {
    ...
    Object.keys(primaryMap).forEach(function(missionId) {
        ...
        merged[missionId] = {
            ...
            lastCompletedAtMs: Math.max(current.lastCompletedAtMs || 0, previous.lastCompletedAtMs || 0)
        };
        if (!Number.isFinite(merged[missionId].completedAtMs) || merged[missionId].completedAtMs === Number.MAX_SAFE_INTEGER) {
            merged[missionId].completedAtMs = Math.max(current.completedAtMs || 0, previous.completedAtMs || 0);
        }
    });
    return merged;
}
...
function normalizeStoryOwnedPieceSets(value) {
    var owned = Array.isArray(value) ? value.filter(Boolean).map(function(item) {
...
function buildStoryProfileState(source) {
    ...
    var active = typeof state.activePieceSet === 'string' && owned.indexOf(state.activePieceSet) !== -1
    ...
    return {
        ...
        storyStateUpdatedAtMs: Math.max(0, parseInt(state.storyStateUpdatedAtMs || 0, 10) || 0)
    };
}
...
function mergeStoryProfileState(primary, fallback) {
    ...
    var preferred = fallbackFreshness > primaryFreshness ? fallbackState : primaryState;
    var secondary = preferred === primaryState ? fallbackState : primaryState;
    var owned = Array.from(new Set([].concat(preferred.storyOwnedPieceSets, secondary.storyOwnedPieceSets, ['neo'])));
    var active = preferred.activePieceSet;
    if (owned.indexOf(active) === -1) {
        active = owned.indexOf(secondary.activePieceSet) !== -1 ? secondary.activePieceSet : 'neo';
    }
    return {
        storyPoints: preferred.storyPoints,
        storyStars: preferred.storyStars,
        storyCompleted: mergeStoryCompletedMap(preferred.storyCompleted, secondary.storyCompleted),
        storyOwnedPieceSets: owned,
        activePieceSet: active,
...
function writeStoryProfileCache(uid, profileData) {
    if (!uid) return;
    try {
        localStorage.setItem(getStoryProfileCacheKey(uid), JSON.stringify(buildStoryProfileState(profileData)));
    } catch (e) {}
}

function normalizeProfileWithStoryState(profileData, fallbackStoryState) {
    var baseProfile = profileData && typeof profileData === 'object' ? Object.assign({}, profileData) : {};
    var nextStoryState = mergeStoryProfileState(baseProfile, fallbackStoryState || readStoryProfileCache(baseProfile.uid || (currentUser && currentUser.uid)));
    return Object.assign(baseProfile, nextStoryState);
}

function syncCurrentProfileStoryState(storyPatch) {
    if (!currentUser) return null;
    currentProfileData = normalizeProfileWithStoryState(Object.assign({}, currentProfileData || {}, storyPatch || {}, {
        uid: currentUser.uid
    }));
...
async function ensureUserProfile() {
    ...
    if (!mergedStoryState.storyStateUpdatedAtMs) mergedStoryState.storyStateUpdatedAtMs = Date.now();
    ...
    currentProfileData = normalizeProfileWithStoryState(Object.assign({}, existing, {
        uid: currentUser.uid,
        displayName: currentUser.displayName || existing.displayName || 'Oyuncu',
        avatar: currentUser.photoURL || existing.avatar || 'fa-chess-pawn',
        email: currentUser.email || existing.email || '',
        friendCode: friendCode,
        friends: Array.isArray(existing.friends) ? existing.friends : []
    }, mergedStoryState), cachedStoryState);
    writeStoryProfileCache(currentUser.uid, currentProfileData);

    if (profileUnsubscribe) profileUnsubscribe();
    profileUnsubscribe = onSnapshot(profileRef, function(profileSnap) {
        currentProfileData = profileSnap.exists()
            ? normalizeProfileWithStoryState(profileSnap.data(), readStoryProfileCache(currentUser && currentUser.uid))
            : null;
        if (currentProfileData && currentUser) writeStoryProfileCache(currentUser.uid, currentProfileData);
        syncFriendProfileSubscriptions();
        updateFriendsSummary();
        renderFriendsView();
        if (window.renderStoryMode && window.currentViewId === 'view-story-mode') window.renderStoryMode();
        if (window.renderStoryShop && window.currentViewId === 'view-story-shop') window.renderStoryShop();
    });
}
...
function getNotificationFeedItems() {
    var derivedFriendRequests = getIncomingPendingFriendRequests().map(function(req) {
        return {
...
function renderNotificationsCenter() {
    ...
    if (!items.length) {
        list.innerHTML = '<div class="friend-empty">Bildirim yok. Yeni istekler, davetler ve DM uyarıları burada görünür.</div>';
        return;
    }
    list.innerHTML = '';
...
window.openNotificationAction = async function(notificationId) {
    ...
    if (action.type === 'analysis_share' && action.shareId) {
        window.closeNotificationsCenter();
        if (window.openSharedAnalysisById) window.openSharedAnalysisById(action.shareId);
        return;
    }
...
function getProfileActivityText(profile) {
    if (!profile || !profile.activeMode || !profile.activeGameStatus) return 'Aktif maç yok';
...
function getFriendLiveWatchData(profile) {
    ...
    var code = profile.activeRoomCode || profile.activeRoomId || profile.activeGameCode || null;
    if (!code) return null;
    return {
...
function getPendingIncomingFriendRequest(targetUid) {
    return friendRequestsCache.find(function(req) {
        return req.status === 'pending' && req.toUid === currentUser.uid && req.fromUid === targetUid;
    }) || null;
}
...
function getIncomingPendingFriendRequests() {
    ...
    return friendRequestsCache.filter(function(req) {
        return req && req.status === 'pending' && req.toUid === currentUser.uid && req.fromUid !== currentUser.uid;
    });
}
...
function renderFriendsList() {
    ...
    friendIds.forEach(function(uid) {
        ...
        if (canWatch) {
            ...
            row.setAttribute('aria-label', (profile.displayName || 'Arkadaş') + ' canlı maçta, izlemek için tıkla');
        }
...
function renderFriendProfile(uid, profile, stats) {
    ...
    var lastMatchesHtml = stats.lastMatches.length
        ? stats.lastMatches.map(function(item) {
            return '<div class="profile-line"><span>' + escapeHtml(item.bucket + ' maçı') + '</span><strong>' + escapeHtml(item.result) + '</strong></div>';
        }).join('')
...
window.openSelectedFriendProfile = function() {
    if (!selectedFriendUid) return window.showToast('Once bir arkadas sec.', 'error');
...
window.watchFriendGame = function(uid) {
    ...
    var watchData = getFriendLiveWatchData(profile);
    if (!watchData) {
        return window.showToast('Bu arkadaş şu anda izlenebilir bir maçta değil.', 'info');
    }
...
window.copyFriendCode = () => {
    if (!currentProfileData || !currentProfileData.friendCode) return window.showToast('Kod henüz hazır değil.', 'error');
...
window.sendFriendRequestByCode = async () => {
    ...
    if (friendCode === currentProfileData.friendCode) return window.showToast('Kendi kodunu giremezsin.', 'error');

    try {
        var matches = await getDocs(query(collection(db, 'profiles'), where('friendCode', '==', friendCode)));
        if (matches.empty) return window.showToast('Bu koda sahip kullanıcı bulunamadı.', 'error');
...
window.acceptFriendRequest = async (requestId) => {
    ...
    if (!request) return;
    if (!currentUser || request.toUid !== currentUser.uid || request.fromUid === currentUser.uid) {
...
window.declineFriendRequest = async (requestId) => {
    var request = friendRequestsCache.find(function(req) { return req.id === requestId; });
    if (request && (!currentUser || request.toUid !== currentUser.uid || request.fromUid === currentUser.uid)) {
        window.showToast('Bu istek sana ait deÄŸil.', 'error');
        return;
    }
    try {
...
if (dmInputArea) {
    dmInputArea.onsubmit = async function(e) {
        ...
        if (!throttleAction('dm_send', selectedFriendUid, 4, 12000)) {
            return window.showToast('DM gonderme limiti doldu. Biraz bekle.', 'error');
        }
        try {
...
function buildLobbyInviteContext(mode) {
    if (mode === '2v2' && window.current2v2Id && window.current2v2Data && window.current2v2Data.status === 'lobby') {
        return { type: '2v2', roomId: window.current2v2Id, roomCode: window.current2v2Data.code || window.current2v2Id, roomLabel: '2v2 Lobi' };
    }
    if (mode === '1v1' && window.current1v1Id && window.current1v1Data && window.current1v1Data.status === 'lobby') {
        return { type: '1v1', roomId: window.current1v1Id, roomCode: window.current1v1Data.code || window.current1v1Id, roomLabel: '1v1 Lobi' };
    }
    if (mode === 'quiz' && window.currentQuizId && window.currentQuizData && window.currentQuizData.status === 'lobby') {
        return { type: 'quiz', roomId: window.currentQuizId, roomCode: window.currentQuizData.code || window.currentQuizId, roomLabel: window.currentQuizData.name || 'Quiz Lobisi' };
    }
    if (mode === 'tournament' && window.currentTournamentId && window.currentTournamentData && window.currentTournamentData.status === 'lobby') {
        return { type: 'tournament', roomId: window.currentTournamentId, roomCode: window.currentTournamentId, roomLabel: window.currentTournamentData.name || 'Turnuva Lobisi' };
    }
    return null;
}
...
window.inviteSelectedFriendToCurrentLobby = function() {
    if (!selectedFriendUid) return window.showToast('Önce bir arkadaş seç.', 'error');
...
window.openLobbyInvites = async function(mode) {
    ...
    if (!buildLobbyInviteContext(mode)) return window.showToast('Bu ekranda aktif bir lobi bulunmuyor.', 'error');
    if (!onlineFriends.length) return window.showToast('Şu an çevrim içi arkadaş yok.', 'info');
...
window.acceptFriendInvite = async function(inviteId) {
    ...
    try {
        ...
        else if (invite.type === 'tournament') window.enterTournament(invite.roomId);
    } catch (e) {
        console.error(e);
...
window.hideChat = () => {
    ...
    if(cb && (window.currentTournamentId || window.current2v2Id || window.current1v1Id)) {
